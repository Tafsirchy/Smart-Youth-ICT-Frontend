...
export default function SocialCreativesDetailsPage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        console.error("Failed to fetch Social Creatives details", err);
      } finally {
        setLoading(false);
      }
    };
  ...
  if (loading) return <div className="min-h-screen bg-slate-50 flex items-center justify-center font-black animate-pulse text-rose-400">LOADING CREATIVE MANIFEST...</div>;
    ...
    <section className="min-h-screen bg-slate-50 text-slate-900 selection:bg-rose-600 selection:text-white pb-40">
      ...
      <div className="border-b border-slate-200 bg-white/80 backdrop-blur-md sticky top-0 z-50">
        <div className="container-custom py-4 flex items-center justify-between">
          ...
          <div className="text-[10px] font-black uppercase tracking-[0.4em] text-rose-600">CREATIVE_SPEC_v2.1</div>
        </div>
      </div>

      <div className="container-custom pt-20">
        <div className="max-w-5xl mb-32">
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex items-center gap-4 text-rose-600 mb-8">
        ...
        <div className="text-center py-40 border-t border-slate-200">
           ...
           <h3 className="text-5xl lg:text-7xl font-black text-slate-900 mb-12 leading-tight">{cta.title?.split('your ')[0]}your <br/><span className="text-transparent bg-clip-text bg-gradient-to-r from-rose-600 to-amber-500 font-serif italic font-medium">{cta.title?.split('your ')[1]}</span></h3>
