...
export default function EcommerceDetailsPage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        console.error("Failed to load ecommerce details", err);
      } finally {
        setLoading(false);
      }
    };
  ...
  if (loading) return <div className="min-h-screen bg-slate-50 flex items-center justify-center font-black animate-pulse text-rose-500 uppercase tracking-widest">ACTIVATING DETAIL PROTOCOL...</div>;
  if (!data) return null;
