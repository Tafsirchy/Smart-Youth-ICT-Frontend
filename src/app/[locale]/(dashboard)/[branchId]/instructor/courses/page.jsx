...
import { 
  HiOutlineAcademicCap, HiOutlineUserGroup, HiOutlineCalendar, 
  HiChevronRight, HiOutlineExternalLink 
} from 'react-icons/hi';
...
export default function InstructorCoursesPage() {
     ...
     return (
       <div className="py-8 space-y-8 animate-pulse">
         ...
         <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
           {[1, 2, 3].map(i => <div key={i} className="h-64 bg-neutral-100 rounded-[40px]" />)}
         </div>
       </div>
     );
  }
  ...
  return (
    <div className="py-8 max-w-6xl">
      <div className="mb-10">
        <h1 className="text-3xl font-extrabold text-neutral-900 tracking-tight">My Assigned Courses</h1>
        <p className="mt-2 text-neutral-500">Manage your course curriculum and monitor student engagement for each module.</p>
      </div>
      ...
      {courses.length > 0 ? (
          ...
          {courses.map((course, i) => (
            ...
            >
              ...
              <div className="flex-1">
                 ...
                 <div className="space-y-3 mb-8">
                    <div className="flex items-center gap-3 text-neutral-400">
                       ...
                       <span className="text-xs font-bold">{course.totalStudents || 0} Registered Students</span>
                    </div>
                    <div className="flex items-center gap-3 text-neutral-400">
                       <HiOutlineCalendar size={18} />
                       <span className="text-xs font-bold">Created: {new Date(course.createdAt).toLocaleDateString()}</span>
                    </div>
                 </div>
              </div>
              ...
              <div className="pt-6 border-t border-neutral-50">
                 ...
                 <a 
                   href={`/courses/${course.slug}`} target="_blank"
        ...
        <div className="bg-white rounded-[40px] border-2 border-dashed border-neutral-100 p-20 text-center">
           ...
           <p className="text-neutral-400 font-medium font-italic">No courses have been assigned to you yet.</p>
        </div>
      )}
    </div>
  );
