...
import { IoPersonOutline, IoCameraOutline, IoSaveOutline } from 'react-icons/io5';

export default function StudentProfilePage() {
  const { data: session, update } = useSession();
  const [form, setForm] = useState({ name: '', phone: '', language: 'en', bio: '' });
  ...
  useEffect(() => {
    api.get('/auth/me').then(res => {
      ...
      setForm({ name: u.name || '', phone: u.phone || '', language: u.language || 'en', bio: u.bio || '' });
  ...
  const handleSave = async (e) => {
    ...
    try {
      ...
      await update({ name: form.name }); // refresh NextAuth session
  ...
  return (
    <div className="pb-10 max-w-2xl">
      <div className="mb-8">
        ...
        <p className="text-textSecondary text-sm mt-1">Manage your personal information and preferences.</p>
      </div>
      ...
      <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} className="space-y-6">
        ...
        <div className="bg-white rounded-2xl ring-1 ring-neutral-200 shadow-sm p-6 flex items-center gap-5">
          <div className="relative">
            <div className="w-20 h-20 rounded-2xl bg-gradient-to-br from-blue-500 to-indigo-600 flex items-center justify-center text-white shrink-0">
              {session?.user?.image
                ? <img src={session.user.image} alt="avatar" className="w-20 h-20 rounded-2xl object-cover" />
          ...
          <div>
            <p className="font-bold text-textPrimary text-lg">{session?.user?.name || 'Student'}</p>
        ...
        {/* Edit form */}
        <form onSubmit={handleSave} className="bg-white rounded-2xl ring-1 ring-neutral-200 shadow-sm p-6 space-y-5">
          {loading ? (
            <div className="space-y-4 animate-pulse">
              {[1,2,3,4].map(n => <div key={n} className="h-12 bg-neutral-200 rounded-xl" />)}
            </div>
          ) : (
            <>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  ...
                  <input className="input w-full" type="text" required placeholder="Your full name"
                    value={form.name} onChange={e => setForm({ ...form, name: e.target.value })} />
                </div>
                <div>
                  <label className="block text-sm font-medium text-textPrimary mb-1.5">Phone (bKash / Nagad)</label>
                  <input className="input w-full" type="tel" placeholder="01XXXXXXXXX"
                    value={form.phone} onChange={e => setForm({ ...form, phone: e.target.value })} />
                </div>
              </div>

              <div>
                <label className="block text-sm font-medium text-textPrimary mb-1.5">Preferred Language</label>
              ...
              <div>
                <label className="block text-sm font-medium text-textPrimary mb-1.5">Short Bio</label>
                <textarea rows={3} className="input w-full resize-none" placeholder="Tell us about yourself…"
