...
import { LuCheck, LuX, LuMessageSquare, LuStar, LuEye, LuPlus, LuPencil, LuTrash2 } from "react-icons/lu";
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editingTestimonial) {
        await api.put(`/cms/testimonials/${editingTestimonial._id}`, submissionData);
    ...
    <div className="p-8">
      <div className="flex justify-between items-center mb-12">
        <div>
          ...
          <p className="text-slate-500">Review, approve, or manually add customer reviews for the marketing site.</p>
        </div>
        <button 
      ...
      <div className="space-y-6">
            ...
            >
              <div className="flex items-center gap-4 w-full md:w-64 shrink-0">
                 <div className="w-14 h-14 rounded-2xl bg-white shadow-sm border border-slate-100 p-0.5 flex-shrink-0">
                   <img src={(t.isManual ? t.manualAvatar : t.user?.avatar) || "/images/placeholder.png"} className="w-full h-full rounded-[1.25rem] object-cover" />
                 </div>
                 <div className="min-w-0">
                   <h4 className="font-bold text-slate-900 truncate">{t.isManual ? t.manualName : t.user?.name}</h4>
                   <p className="text-[10px] font-black uppercase text-brand-green truncate">
                     {t.isManual ? t.manualCourse : (t.course?.title?.en || t.course?.title || "Course Review")}
                   </p>
                 </div>
              </div>
              ...
              <div className="flex-1">
                <div className="flex gap-1 text-amber-400 mb-3">
                  {[...Array(5)].map((_, i) => (
                    <LuStar key={i} className={`w-4 h-4 ${i < t.rating ? 'fill-current' : 'text-slate-100'}`} />
                  ))}
                </div>
                <p className="text-slate-600 text-sm leading-relaxed mb-6 font-medium italic">“{t.text}”</p>
                {t.incomeProof && (
                  <div className="flex gap-4 items-center">
                    <a href={t.incomeProof} target="_blank" className="inline-flex items-center gap-2 px-4 py-2 bg-slate-100 text-slate-600 text-[10px] font-black uppercase tracking-wider rounded-xl hover:bg-slate-200 transition-colors">
        ...
        {!loading && testimonials.length === 0 && (
          <div className="py-20 text-center flex flex-col items-center">
            ...
            <p className="text-slate-400 font-bold uppercase tracking-widest text-[10px]">No testimonials to moderate</p>
          </div>
        )}
      </div>
      ...
      {showModal && (
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="bg-white w-full max-w-2xl rounded-[3rem] p-10 shadow-2xl relative max-h-[90vh] overflow-y-auto">
            ...
            <h3 className="text-2xl font-black text-slate-900 mb-8">{editingTestimonial ? "Edit Feedback" : "New Testimonial"}</h3>
            <form onSubmit={handleSubmit} className="space-y-6">
               <div className="flex gap-8">
                  <ImageUpload 
                    value={formData.isManual ? formData.manualAvatar : (formData.user?.avatar || "")} 
                    onChange={(url) => setFormData({...formData, [formData.isManual ? 'manualAvatar' : 'user']: formData.isManual ? url : {...formData.user, avatar: url}})} 
                    label="User Avatar"
                  />
                  
                  <div className="flex-1 space-y-4">
                    <div className="flex justify-between items-center bg-slate-50 p-3 rounded-2xl border border-slate-100">
                      <span className="text-[10px] font-black uppercase text-slate-400">Entry Mode</span>
                      <button 
                        type="button"
                    ...
                    <div>
                      <label className="block text-[10px] font-black uppercase text-slate-400 mb-1">
                        {formData.isManual ? 'Student Name' : 'User Name (Read-Only)'}
                      </label>
                      <input 
                        required 
                        ...
                        value={formData.isManual ? formData.manualName : (formData.user?.name || "")} 
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      <div>
                        <label className="block text-[10px] font-black uppercase text-slate-400 mb-1">Rating (1-5)</label>
                        <select className="w-full px-5 py-3.5 bg-slate-50 border-2 border-transparent focus:border-indigo-500/20 focus:bg-white transition-all rounded-2xl outline-none text-sm font-bold shadow-inner" value={formData.rating} onChange={(e) => setFormData({...formData, rating: Number(e.target.value)})}>
                          {[5,4,3,2,1].map(r => <option key={r} value={r}>{r} Stars</option>)}
                        </select>
                      </div>
                      <div>
                        <label className="block text-[10px] font-black uppercase text-slate-400 mb-1">Course Title</label>
                        {formData.isManual ? (
                          <input 
                            required 
                            ...
                            onChange={(e) => setFormData({...formData, manualCourse: e.target.value})} 
                          />
                        ) : (
                          <select className="w-full px-5 py-3.5 bg-slate-50 border-2 border-transparent focus:border-indigo-500/20 focus:bg-white transition-all rounded-2xl outline-none text-sm font-bold shadow-inner" value={formData.course} onChange={(e) => setFormData({...formData, course: e.target.value})}>
               ...
               <div>
                 <label className="block text-[10px] font-black uppercase text-slate-400 mb-1">Review Text</label>
                 <textarea rows="4" required className="w-full px-5 py-3.5 bg-slate-50 border-2 border-transparent focus:border-indigo-500/20 focus:bg-white transition-all rounded-2xl outline-none text-sm resize-none shadow-inner" placeholder="Enter the user's detailed feedback..." value={formData.text} onChange={(e) => setFormData({...formData, text: e.target.value})} />
               </div>

               <ImageUpload 
                  value={formData.incomeProof} 
                  onChange={(url) => setFormData({...formData, incomeProof: url})} 
