...
import { 
  ...
  IoLayersOutline, IoSettingsOutline, IoShapesOutline, IoCodeSlashOutline, 
  IoColorWandOutline, IoCubeOutline, IoFlashOutline, IoPulseOutline
} from "react-icons/io5";
import api from "@/lib/api";
import toast from "react-hot-toast";

const ICON_OPTIONS = ["Palette", "Diamond", "Flash", "Globe", "Shield", "Rocket", "Briefcase", "Stats", "Chip", "Infinite", "Sparkles", "FingerPrint", "Flask", "Images", "ShareSocial", "Shapes", "TrendingUp", "Search", "ColorWand", "GitNetwork"];
...
export default function UiUxCMS() {
  ...
  const handleSave = async () => {
    setSaving(true);
    try {
      await api.put("/cms/services/web-software/ui-ux", content);
      toast.success("UI/UX content updated globally");
    } catch (err) {
  ...
  if (loading) return <div className="p-20 text-center font-black animate-pulse text-cyan-500">LOADING DESIGN ENGINE...</div>;

  return (
    <div className="p-8 max-w-7xl mx-auto pb-40">
      <div className="flex justify-between items-center mb-12">
        <div>
          <h1 className="text-4xl font-black text-slate-900 mb-2">UI/UX Design CMS</h1>
          <p className="text-slate-500 font-bold uppercase tracking-widest text-[10px]">Managing Product Experience</p>
        </div>
        <button 
          onClick={handleSave}
          disabled={saving}
      ...
      <div className="flex gap-4 mb-12 border-b border-slate-100 pb-4">
        {[
          { id: "landing", label: "Landing Architecture", icon: IoLayersOutline, color: "text-cyan-500" },
          ...
          <motion.div key="landing" initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 20 }} className="space-y-12">
            ...
            <section className="bg-white p-10 rounded-[3rem] border border-slate-100 shadow-sm">
                <h2 className="text-xl font-black uppercase tracking-tighter mb-8 flex items-center gap-3">
                    <IoColorWandOutline className="text-cyan-500" /> Hero Section
                </h2>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                    <div>
                        <label className="block text-[10px] font-black uppercase text-slate-400 mb-2 font-bold">Badge Text</label>
                        <input className="w-full px-5 py-3 bg-slate-50 border border-transparent focus:border-cyan-500/20 rounded-xl outline-none font-bold text-sm" value={content.landing.hero.badge || ""} onChange={(e) => updateNested("landing", "hero.badge", e.target.value)} />
                    </div>
                    <div>
                        <label className="block text-[10px] font-black uppercase text-slate-400 mb-2 font-bold">Primary Title</label>
                        <input className="w-full px-5 py-3 bg-slate-50 border border-transparent focus:border-cyan-500/20 rounded-xl outline-none font-bold text-sm" value={content.landing.hero.title || ""} onChange={(e) => updateNested("landing", "hero.title", e.target.value)} />
                    </div>
                    <div className="md:col-span-2">
                        <label className="block text-[10px] font-black uppercase text-slate-400 mb-2 font-bold">Hero Description</label>
                        <textarea rows="3" className="w-full px-5 py-3 bg-slate-50 border border-transparent focus:border-cyan-500/20 rounded-xl outline-none font-medium text-sm leading-relaxed" value={content.landing.hero.description || ""} onChange={(e) => updateNested("landing", "hero.description", e.target.value)} />
                    </div>
                </div>
            </section>
            ...
            <section className="bg-white p-10 rounded-[3rem] border border-slate-100 shadow-sm">
                <div className="flex justify-between items-center mb-8">
                    <h2 className="text-xl font-black uppercase tracking-tighter flex items-center gap-3">
                        <IoShapesOutline className="text-cyan-500" /> Design Pillars
                    </h2>
                    <button onClick={() => {
                        const newPillars = [...(content.landing.sections.pillars || []), { title: "", desc: "", icon: "Layers", color: "from-cyan-500 to-blue-600" }];
                        updateNested("landing", "sections.pillars", newPillars);
                    }} className="text-[10px] font-black uppercase text-cyan-500 hover:text-cyan-600">+ Add Pillar</button>
                </div>
            ...
            <section className="bg-white p-10 rounded-[3rem] border border-slate-100 shadow-sm font-black">
                <div className="flex justify-between items-center mb-8">
                    <h2 className="text-xl font-black uppercase tracking-tighter flex items-center gap-3">
                        <IoCubeOutline className="text-cyan-500" /> Atomic Assets
                    </h2>
                    <button onClick={() => {
                        const newMetrics = [...(content.landing.sections.metrics || []), { t: "" }];
                        updateNested("landing", "sections.metrics", newMetrics);
                    }} className="text-[10px] font-black uppercase text-cyan-500 hover:text-cyan-600">+ Add Asset</button>
                </div>
          ...
          <motion.div key="details" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-12">
            ...
            <section className="bg-white p-10 rounded-[3rem] border border-slate-100 shadow-sm">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                    ...
                    <div>
                        <label className="block text-[10px] font-black uppercase text-slate-400 mb-2 font-bold">Design Title</label>
            ...
            <section className="bg-white p-10 rounded-[3rem] border border-slate-100 shadow-sm text-slate-900">
                <div className="flex justify-between items-center mb-8">
                    <h2 className="text-xl font-black uppercase tracking-tighter flex items-center gap-3">
                        <IoPulseOutline className="text-indigo-500" /> Performance ROI
                    </h2>
                    <button onClick={() => {
