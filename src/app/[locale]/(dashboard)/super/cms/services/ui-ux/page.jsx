      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-6">
        <div>
          <div className="flex items-center gap-2 text-cyan-600 mb-1">
            ...
            <span className="text-[10px] font-black uppercase tracking-[0.4em]">Service_Tier_Product_Design</span>
          </div>
          <h1 className="text-3xl font-black text-slate-900 tracking-tighter leading-none">UI/UX Design</h1>
          <p className="text-slate-400 font-bold uppercase tracking-widest text-[10px] mt-1.5 ml-1">Centralized Experience Node</p>
        </div>
        <button 
          onClick={handleSave}
          disabled={saving}
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                    {content.landing.sections.pillars?.map((item, idx) => (
                        <CardWrapper key={idx} onRemove={() => {
                            ...
                            updateNested("landing", "sections.pillars", newArr);
                             ...
                             <div className="flex gap-4 mb-4">
                                ...
                                <div className="flex-1 min-w-0">
                                    <input placeholder="Process" className="w-full bg-transparent border-none outline-none font-black text-sm uppercase tracking-tighter" value={item.title} onChange={(e) => {
                                        const newArr = [...content.landing.sections.pillars];
                            ...
                            <textarea placeholder="Psychological rationale" rows="3" className="w-full bg-slate-50 border border-transparent focus:bg-white focus:border-slate-100 rounded-xl p-3 text-[11px] text-slate-500 leading-relaxed outline-none transition-all scrollbar-hide" value={item.desc} onChange={(e) => {
                                const newArr = [...content.landing.sections.pillars];
            ...
            <section className="bg-slate-900 p-6 rounded-3xl text-white shadow-xl">
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3">
                    {content.landing.sections.metrics?.map((metric, idx) => (
                        <div key={idx} className="bg-white/5 border border-white/10 p-4 rounded-xl relative group hover:bg-white/10 transition-all text-center">
                             <button onClick={() => {
                                const newArr = content.landing.sections.metrics.filter((_, i) => i !== idx);
                                updateNested("landing", "sections.metrics", newArr);
                            }} className="absolute top-2 right-2 text-white/10 hover:text-rose-500 opacity-0 group-hover:opacity-100 transition-all"><IoTrashOutline size={12}/></button>
                            <input className="bg-transparent border-none outline-none font-black text-[10px] uppercase text-white w-full text-center" value={metric.t} onChange={(e) => {
                    ...
                    <button onClick={() => {
                        ...
                        updateNested("landing", "sections.metrics", newArr);
                    }} className="border-2 border-dashed border-white/10 rounded-xl flex items-center justify-center py-3 text-white/20 hover:text-cyan-400 hover:bg-white/5 transition-all">
                    ...
                    {content.details.sections.phases?.map((phase, idx) => (
                        <div key={idx} className="p-5 bg-slate-50 rounded-2xl relative group border border-transparent hover:border-cyan-100 transition-all">
                            ...
                            <div className="flex gap-3 mb-3">
                                <input placeholder="ID" className="w-16 bg-white border border-slate-100 rounded-lg px-2 py-1 text-[9px] font-black text-cyan-600 uppercase" value={phase.step} onChange={(e) => {
                                    const newArr = [...content.details.sections.phases];
                                    newArr[idx].step = e.target.value;
                                    updateNested("details", "sections.phases", newArr);
                                }} />
                                <input placeholder="Stage Name" className="flex-1 bg-transparent border-none outline-none font-black text-[10px] tracking-widest uppercase text-slate-900 focus:text-cyan-600 transition-colors" value={phase.stage} onChange={(e) => {
                                    const newArr = [...content.details.sections.phases];
                                    newArr[idx].stage = e.target.value;
                                    updateNested("details", "sections.phases", newArr);
                                }} />
                            </div>
                            <textarea placeholder="Process description" rows="3" className="w-full bg-transparent border-none outline-none text-[10px] text-slate-500 font-medium leading-relaxed scrollbar-hide shrink-0" value={phase.action} onChange={(e) => {
                                const newArr = [...content.details.sections.phases];
                        ...
                        <div key={idx} className="p-6 bg-slate-50 rounded-3xl relative group border border-slate-100 shadow-sm">
                            ...
                            <div className="space-y-2">
                                ...
                                <button onClick={() => {
                                    ...
                                    updateNested("details", "sections.roi", newArr);
                                }} className="text-[10px] font-black text-cyan-400 uppercase tracking-widest mt-2 hover:text-cyan-600">+ Add Objective</button>
                            </div>
                        </div>
                    ))}
                </div>
