...
import { 
  ...
  IoSearchOutline, IoFlashOutline, IoCubeOutline
} from "react-icons/io5";
import api from "@/lib/api";
import toast from "react-hot-toast";

const ICON_OPTIONS = [
  "Search", "Layers", "Flash", "Cube", "Shield", "Globe", "Analytics", "Stats", "Rocket", "Palette"
];

const COLOR_OPTIONS = [
  { name: "Cyan", value: "from-cyan-600 to-indigo-600", text: "text-cyan-600" },
  { name: "Indigo", value: "from-indigo-600 to-blue-700", text: "text-indigo-600" },
  { name: "Rose", value: "from-rose-500 to-pink-600", text: "text-rose-500" },
  { name: "Slate", value: "from-slate-700 to-slate-900", text: "text-slate-700" }
];

export default function UIUXCMS() {
  ...
  const fetchData = async () => {
    try {
      const res = await api.get("/cms/services/creative-marketing/ui-ux");
      if (res.data.data) {
        setContent(prev => ({
            ...prev,
            ...res.data.data,
      ...
      toast.error("Failed to load UI/UX content");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSave = async () => {
    setSaving(true);
    try {
      await api.put("/cms/services/creative-marketing/ui-ux", content);
      toast.success("Product experience updated globally");
    } catch (err) {
  ...
  if (loading) return (
    <div className="min-h-screen bg-slate-50 flex flex-col items-center justify-center p-10 text-center space-y-2">
        ...
        <span className="font-black text-slate-900 uppercase tracking-widest text-xs">Initializing Product Design Lab...</span>
    </div>
  );
  ...
  return (
    <div className="min-h-screen bg-slate-50 p-4 max-w-7xl mx-auto pb-6 text-slate-900 selection:bg-cyan-600 selection:text-white">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-3 mb-6">
        <div>
          <div className="flex items-center gap-2 text-cyan-600 mb-1">
            ...
            <span className="text-[9px] font-black uppercase tracking-[0.4em]">Service_Tier_Product_Design</span>
          </div>
          <h1 className="text-3xl font-black text-slate-900 tracking-tighter leading-none">UI/UX Design</h1>
          <p className="text-slate-400 font-bold uppercase tracking-widest text-[9px] mt-1">Centralized Experience Node</p>
        </div>
        <button 
          onClick={handleSave}
          disabled={saving}
       ...
       <div className="flex gap-1 mb-6 bg-white p-1 rounded-xl border border-slate-200 w-fit shadow-sm">
        {[
          { id: "landing", label: "Experience Landing", icon: IoLayersOutline, color: "text-cyan-500" },
          { id: "details", label: "Component Manifest", icon: IoSettingsOutline, color: "text-indigo-500" },
        ].map(tab => (
          <button
            key={tab.id}
        ...
        {activeTab === "landing" ? (
            ...
            <section className="bg-white p-5 rounded-3xl border border-slate-100 shadow-lg shadow-slate-200/30">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                    ...
                    <div className="md:col-span-2">
                        <Field label="Design Philosophy" value={content.landing.hero.description} onChange={(v) => updateNested("landing", "hero.description", v)} textarea />
                    </div>
                </div>
            </section>
             ...
             <section className="bg-white p-5 rounded-3xl border border-slate-100 shadow-lg shadow-slate-200/30">
                <div className="flex justify-between items-center mb-4 pb-2 border-b border-slate-100">
                    <h2 className="text-xl font-black tracking-tighter flex items-center gap-2">
                        <div className="w-8 h-8 bg-cyan-50 rounded-lg flex items-center justify-center text-cyan-600"><IoCubeOutline /></div>
                        Design Pillars
                    </h2>
                </div>
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
                    {content.landing.sections.pillars?.map((item, idx) => (
                        <CardWrapper key={idx} onRemove={() => {
                            ...
                            updateNested("landing", "sections.pillars", newArr);
                             ...
                             <div className="flex gap-3 mb-3">
                                ...
                                <div className="flex-1 min-w-0">
                                    <input placeholder="Process" className="w-full bg-transparent border-none outline-none font-black text-base md:text-sm uppercase tracking-tighter" value={item.title} onChange={(e) => {
                                        const newArr = [...content.landing.sections.pillars];
                            ...
                            <textarea placeholder="Psychological rationale" rows="3" className="w-full bg-slate-50 border border-transparent focus:bg-white focus:border-slate-100 rounded-xl p-2 text-base md:text-[11px] text-slate-500 leading-relaxed outline-none transition-all scrollbar-hide" value={item.desc} onChange={(e) => {
                                const newArr = [...content.landing.sections.pillars];
                    ...
                    <AddButton onClick={() => {
                        const newArr = [...(content.landing.sections.pillars || []), { title: "New Process", desc: "", icon: "Search", color: "from-cyan-600 to-indigo-600" }];
                        updateNested("landing", "sections.pillars", newArr);
                    }} label="Add Pillar" />
                </div>
            </section>
            ...
            <section className="bg-slate-900 p-5 rounded-3xl text-white shadow-xl">
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3">
                    {content.landing.sections.metrics?.map((metric, idx) => (
                        <div key={idx} className="bg-white/5 border border-white/10 p-3 rounded-xl relative group hover:bg-white/10 transition-all text-center">
                             <button onClick={() => {
                                const newArr = content.landing.sections.metrics.filter((_, i) => i !== idx);
                                updateNested("landing", "sections.metrics", newArr);
                            }} className="absolute top-2 right-2 text-white/10 hover:text-rose-500 opacity-0 group-hover:opacity-100 transition-all p-1 bg-white/5 rounded-lg"><IoTrashOutline size={12}/></button>
                            <input className="bg-transparent border-none outline-none font-black text-base md:text-[10px] uppercase text-white w-full text-center" value={metric.t} onChange={(e) => {
                    ...
                    <button onClick={() => {
                        const newArr = [...(content.landing.sections.metrics || []), { t: "NEW_TOKEN" }];
                        updateNested("landing", "sections.metrics", newArr);
                    }} className="border-2 border-dashed border-white/10 rounded-xl flex items-center justify-center py-2 text-white/20 hover:text-cyan-400 hover:bg-white/5 transition-all">
          ...
          <motion.div key="details" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="space-y-6">
            ...
            <section className="bg-white p-5 rounded-3xl border border-slate-100 shadow-lg shadow-slate-200/30">
                <div className="flex justify-between items-center mb-4 pb-2 border-b border-slate-100">
                    <h2 className="text-xl font-black tracking-tighter flex items-center gap-2">
                        <div className="w-8 h-8 bg-cyan-50 rounded-lg flex items-center justify-center text-cyan-600"><IoGitNetworkOutline /></div>
                        The Design Protocol (Lifecycle)
                    </h2>
                    <AddButton onClick={() => {
                        const newArr = [...(content.details.sections.phases || []), { step: "DESIGN_0X", stage: "", action: "" }];
                    ...
                    {content.details.sections.phases?.map((phase, idx) => (
                        <div key={idx} className="p-4 bg-slate-50 rounded-2xl relative group border border-transparent hover:border-cyan-100 transition-all">
                            ...
                            <div className="flex gap-2 mb-2">
                                <input placeholder="ID" className="w-16 bg-white border border-slate-100 rounded-lg px-2 py-1 text-base md:text-[9px] font-black text-cyan-600 uppercase" value={phase.step} onChange={(e) => {
                                    const newArr = [...content.details.sections.phases];
                                    newArr[idx].step = e.target.value;
                                    updateNested("details", "sections.phases", newArr);
                                }} />
                                <input placeholder="Stage Name" className="flex-1 bg-transparent border-none outline-none font-black text-base md:text-[10px] tracking-widest uppercase text-slate-900 focus:text-cyan-600 transition-colors" value={phase.stage} onChange={(e) => {
                                    const newArr = [...content.details.sections.phases];
                                    newArr[idx].stage = e.target.value;
                                    updateNested("details", "sections.phases", newArr);
                                }} />
                            </div>
                            <textarea placeholder="Process description" rows="3" className="w-full bg-transparent border-none outline-none text-base md:text-[10px] text-slate-500 font-medium leading-relaxed scrollbar-hide shrink-0" value={phase.action} onChange={(e) => {
                                const newArr = [...content.details.sections.phases];
                        ...
                        <div key={idx} className="p-4 bg-slate-50 rounded-3xl relative group border border-slate-100 shadow-sm">
                            ...
                            <div className="space-y-1">
                                ...
                                <button onClick={() => {
                                    ...
                                    updateNested("details", "sections.roi", newArr);
                                }} className="text-[10px] font-black text-cyan-400 uppercase tracking-widest mt-1 hover:text-cyan-600">+ Add Objective</button>
                            </div>
                        </div>
                    ))}
                </div>
      ...
      <div className="md:hidden sticky bottom-0 left-0 w-full pt-4 pb-[calc(1rem+env(safe-area-inset-bottom))] bg-slate-50/90 backdrop-blur-md z-[90]">
        ...
        >
          <div className="absolute inset-0 bg-gradient-to-r from-cyan-600 to-indigo-600 opacity-0 hover:opacity-100 transition-opacity" />
          <div className="relative flex items-center gap-2">
...
function CardWrapper({ children, onRemove, highlight = false, compact = false }) {
    return (
        <div className={`p-4 bg-white border border-slate-100 rounded-2xl shadow-sm hover:shadow-lg transition-all relative group ${highlight ? 'ring-2 ring-cyan-600 shadow-cyan-600/10' : ''}`}>
            {onRemove && (
                <button 
                    onClick={onRemove} 
...
function AddButton({ onClick, label = "Add", small = false }) {
    if (small) return (
        <button onClick={onClick} className="p-1.5 bg-cyan-50 text-cyan-600 rounded-lg hover:bg-cyan-600 hover:text-white transition-all">
    ...
    return (
        <button 
            onClick={onClick}
            className="h-full min-h-[80px] border-2 border-dashed border-slate-100 rounded-2xl flex flex-col items-center justify-center gap-1 text-slate-300 hover:text-cyan-500 hover:border-cyan-200 hover:bg-cyan-50/30 transition-all group p-2"
        >
