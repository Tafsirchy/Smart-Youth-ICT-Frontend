...
import { 
  ...
  IoRibbonOutline, IoFlashOutline, IoShieldCheckmarkOutline as IoShieldC
} from "react-icons/io5";
import api from "@/lib/api";
import toast from "react-hot-toast";

const ICON_OPTIONS = [
  "Ribbon", "Shield", "Flash", "Briefcase", "Globe", "Analytics", "Rocket", "Layers"
];

const COLOR_OPTIONS = [
  { name: "Indigo", value: "from-indigo-600 to-rose-600", text: "text-indigo-600" },
...
export default function CertificationsCMS() {
  ...
  const fetchData = async () => {
    try {
      ...
      toast.error("Failed to load certifications content");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSave = async () => {
    setSaving(true);
    try {
      await api.put("/cms/services/content/certifications", content);
      toast.success("Certification framework updated globally");
    } catch (err) {
            ...
            <section className="bg-white p-8 rounded-3xl border border-slate-100 shadow-lg shadow-slate-200/30">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div className="md:col-span-2">
                        <Field label="Credential Manifesto" value={content.hero.description} onChange={(v) => updateNested("hero.description", v)} textarea />
                    </div>
                </div>
            </section>
          ...
          <motion.div key="methodology" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="space-y-8">
            ...
            <section className="bg-white p-8 rounded-3xl border border-slate-100 shadow-lg shadow-slate-200/30">
                <div className="flex justify-between items-center mb-6 pb-4 border-b border-slate-100">
                    ...
                    <AddButton onClick={() => {
                        const newArr = [...(content.methodology || []), { title: "New Validation Node", description: "" }];
