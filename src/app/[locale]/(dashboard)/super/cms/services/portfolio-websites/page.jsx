    ...
    <div className="min-h-screen bg-slate-50 p-4 max-w-7xl mx-auto pb-6 text-slate-900 selection:bg-rose-600 selection:text-white">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-3 mb-6">
        <div>
          ...
          <h1 className="text-3xl font-black text-slate-900 leading-none">Portfolio Websites</h1>
          <p className="text-slate-400 font-bold uppercase tracking-widest text-[9px] mt-1">Centralized Creative Node</p>
        </div>
        <button 
          onClick={handleSave}
          disabled={saving}
      ...
      <AnimatePresence mode="wait">
        {activeTab === "landing" ? (
          <motion.div key="landing" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="space-y-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
                    {content.landing.sections.philosophies?.map((item, idx) => (
                        <CardWrapper key={idx} onRemove={() => {
                            ...
                            <textarea placeholder="Description" rows="3" className="w-full bg-slate-50 border border-transparent focus:bg-white focus:border-slate-100 rounded-xl p-2 text-base md:text-[11px] text-slate-500 leading-normal outline-none transition-all" value={item.desc} onChange={(e) => {
                                const newArr = [...content.landing.sections.philosophies];
            ...
            <section className="bg-slate-900 p-5 rounded-3xl text-white shadow-xl shadow-rose-900/10">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
                    {content.landing.sections.phases?.map((phase, idx) => (
                        <div key={idx} className="bg-white/5 border border-white/10 p-4 rounded-2xl relative group">
                            ...
                            <textarea placeholder="Phase Description" rows="2" className="w-full bg-transparent border-none outline-none text-base md:text-[11px] text-slate-400 font-medium leading-normal" value={phase.d} onChange={(e) => {
                                const newArr = [...content.landing.sections.phases];
                    ...
                    <button onClick={() => {
                        ...
                        updateNested("landing", "sections.phases", newArr);
                    }} className="h-full min-h-[80px] border-2 border-dashed border-white/10 rounded-2xl flex flex-col items-center justify-center gap-1 text-white/20 hover:text-rose-400 hover:border-rose-400/30 hover:bg-white/5 transition-all group">
                                ...
                                {tier.list?.map((feature, fIdx) => (
                                    <div key={fIdx} className="flex gap-2 group/feat items-center">
                                        ...
                                        <button onClick={() => {
                                            ...
                                            updateNested("landing", "sections.pricing", newArr);
                                        }} className="opacity-0 group-hover/feat:opacity-100 text-rose-500 hover:scale-110 transition-all p-1"><IoTrashOutline size={12}/></button>
                                    </div>
                                ))}
                                <button onClick={() => {
                                    const newArr = [...content.landing.sections.pricing];
                    ...
                    {content.details.sections.techStack?.map((tech, idx) => (
                        <CardWrapper key={idx} onRemove={() => {
                            ...
                            updateNested("details", "sections.techStack", newArr);
                             ...
                             <div className="flex gap-3 mb-3">
                                <select className="bg-slate-50 border border-slate-100 rounded-lg p-1 text-[9px] font-black uppercase tracking-widest outline-none text-blue-600" value={tech.icon} onChange={(e) => {
                            ...
                            <textarea placeholder="Integration Detail" rows="2" className="w-full bg-slate-50 border border-transparent focus:bg-white focus:border-slate-100 rounded-xl p-2 text-base md:text-[11px] text-slate-500 leading-normal outline-none transition-all" value={tech.d} onChange={(e) => {
                                const newArr = [...content.details.sections.techStack];
                                newArr[idx].d = e.target.value;
                                updateNested("details", "sections.techStack", newArr);
                            }} />
                ...
                <div className="grid lg:grid-cols-2 gap-6">
                        ...
                        <div>
                             ...
                             <div className="flex flex-wrap gap-2 p-3 bg-white/5 border border-white/10 rounded-xl">
                                {content.details.sections.codeSnippet?.tags?.map((tag, i) => (
                                    <div key={i} className="flex items-center bg-white/10 px-2 py-1 rounded-lg gap-1">
                                        ...
                                        <button onClick={() => {
                                            ...
                                            updateNested("details", "sections.codeSnippet.tags", newTags);
                                        }} className="text-white/20 hover:text-rose-500 p-1"><IoTrashOutline size={12}/></button>
                                    </div>
                                ))}
                                <button onClick={() => {
                                    const newTags = [...(content.details.sections.codeSnippet.tags || []), "NEW_TAG"];
                                    updateNested("details", "sections.codeSnippet.tags", newTags);
                                }} className="text-[10px] font-black text-blue-400 border border-blue-400/30 px-2 py-1 rounded-lg hover:bg-blue-400/10">+ Add</button>
                             </div>
                        </div>
                    </div>
                    <div className="space-y-3">
                        ...
                        <div className="relative group">
                             <div className="absolute -top-3 right-4 px-2 py-1 bg-blue-600 rounded-full text-[8px] font-black uppercase tracking-widest z-10 shadow-lg">Live_Editor</div>
                             <textarea 
                                rows="10" 
                                className="w-full bg-black/50 border border-white/10 rounded-2xl p-4 font-mono text-base md:text-[11px] text-blue-300 outline-none focus:border-blue-500/30 transition-all scrollbar-hide"
...
function Field({ label, value, onChange, textarea = false, dark = false, small = false, icon = null }) {
    ...
    return (
        <div className={small ? "w-48" : "w-full"}>
            ...
            <div className={`relative flex flex-col ${dark ? 'bg-white/5 border-white/5' : 'bg-slate-50 border-transparent'} border rounded-xl overflow-hidden focus-within:border-rose-500/10 transition-all p-1`}>
...
function AddButton({ onClick, label = "Add", small = false }) {
    if (small) return (
        <button onClick={onClick} className="p-1 bg-rose-50 text-rose-600 rounded-lg hover:bg-rose-600 hover:text-white transition-all">
    ...
    return (
        <button 
            onClick={onClick}
            className="h-full min-h-[80px] border-2 border-dashed border-slate-100 rounded-2xl flex flex-col items-center justify-center gap-1 text-slate-300 hover:text-rose-500 hover:border-rose-200 hover:bg-rose-50/30 transition-all group"
        >
