    ...
    <div className="min-h-screen bg-slate-50 p-4 max-w-7xl mx-auto pb-6 text-slate-900 selection:bg-amber-600 selection:text-white">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-3 mb-6">
        <div>
          ...
          <h1 className="text-3xl font-black text-slate-900 leading-none">Skill Development</h1>
          <p className="text-slate-400 font-bold uppercase tracking-widest text-[9px] mt-1">Centralized Learning Node</p>
        </div>
        <button 
          onClick={handleSave}
          disabled={saving}
                        ...
                        <div key={idx} className="p-4 bg-slate-50 rounded-3xl relative group border border-transparent hover:border-amber-100 transition-all flex flex-col h-full">
                            ...
                            <div className="flex-1 space-y-2">
                                <div>
                                    ...
                                    <input placeholder="Methodology Node" className="w-full bg-white px-2 py-1 rounded-xl border border-slate-100 outline-none font-black text-base md:text-sm uppercase text-slate-900" value={item.title} onChange={(e) => {
                                ...
                                <div>
                                    ...
                                    <textarea placeholder="Scientific explanation" rows="4" className="w-full bg-white px-2 py-1.5 rounded-xl border border-slate-100 outline-none text-base md:text-[11px] text-slate-500 font-medium leading-normal resize-none" value={item.description} onChange={(e) => {
                                        const newArr = [...content.methodology];
      ...
      <div className="md:hidden sticky bottom-0 left-0 w-full pt-4 pb-[calc(1rem+env(safe-area-inset-bottom))] bg-slate-50/90 backdrop-blur-md z-[90]">
        ...
        >
          <div className="absolute inset-0 bg-gradient-to-r from-amber-600 to-orange-600 opacity-0 hover:opacity-100 transition-opacity" />
          <div className="relative flex items-center gap-2">
...
function AddButton({ onClick, label = "Add", small = false }) {
    ...
    return (
        <button 
            onClick={onClick}
            className="h-full min-h-[80px] border-2 border-dashed border-slate-100 rounded-2xl flex flex-col items-center justify-center gap-1 text-slate-300 hover:text-amber-500 hover:border-amber-200 hover:bg-amber-50/30 transition-all group min-h-[80px]"
        >
