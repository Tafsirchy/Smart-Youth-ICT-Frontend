...
export default function ErpCrmCMS() {
  ...
  return (
    <div className="min-h-screen bg-slate-50 p-4 max-w-7xl mx-auto pb-6 text-slate-900 selection:bg-teal-600 selection:text-white">
      {/* HEADER PROTOCOL */}
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-3 mb-6">
        <div>
          <div className="flex items-center gap-2 text-teal-600 mb-1">
            <IoSyncOutline className="text-lg" />
            <span className="text-[9px] font-black uppercase tracking-[0.4em]">Service_Tier_Enterprise_Systems</span>
          </div>
          <h1 className="text-3xl font-black text-slate-900 tracking-tighter leading-none">ERP / CRM / POS Systems</h1>
          <p className="text-slate-400 font-bold uppercase tracking-widest text-[9px] mt-1">Centralized Operational Node</p>
        </div>
        <button 
          onClick={handleSave}
          disabled={saving}
          className="hidden md:flex group relative overflow-hidden px-4 py-2 bg-slate-900 text-white rounded-xl font-black uppercase tracking-widest text-[10px] items-center gap-2 hover:shadow-xl hover:shadow-teal-950/20 transition-all active:scale-95 disabled:opacity-50"
        >
       ...
       <div className="flex gap-1 mb-6 bg-white p-1 rounded-xl border border-slate-200 w-fit shadow-sm">
        {[
          ...
          >
            <tab.icon size={16} className={activeTab === tab.id ? 'text-teal-400' : tab.color} />
            {tab.label}
          </button>
        ))}
      </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                    {content.landing.sections.verticals?.map((item, idx) => (
                        <CardWrapper key={idx} onRemove={() => {
                            ...
                            updateNested("landing", "sections.verticals", newArr);
                             ...
                             <div className="flex gap-3 mb-3">
                                <div className="shrink-0 relative group/icon">
                                    <div className="w-10 h-10 rounded-xl bg-slate-50 flex items-center justify-center text-teal-600 text-xl shadow-inner border border-slate-100 group-hover:scale-105 transition-all">
                            ...
                            <textarea placeholder="Feature Detail" rows="3" className="w-full bg-slate-50 border border-transparent focus:bg-white focus:border-slate-100 rounded-xl p-2 text-base md:text-[11px] text-slate-500 leading-relaxed outline-none transition-all scrollbar-hide" value={item.desc} onChange={(e) => {
                                const newArr = [...content.landing.sections.verticals];
                        ...
                        <div key={idx} className="bg-white/5 border border-white/10 p-4 rounded-2xl relative group">
                             ...
                             <div className="flex flex-wrap gap-2">
                                ...
                                <button onClick={() => {
                                    ...
                                    updateNested("landing", "sections.integrations", newArr);
                                }} className="text-[9px] font-black text-teal-500 border border-teal-500/30 px-2 py-1 rounded-lg hover:bg-teal-500/10">+ Add</button>
                             </div>
                        </div>
                    ))}
                    <button onClick={() => {
                        ...
                        updateNested("landing", "sections.integrations", newArr);
                    }} className="h-full min-h-[90px] border-2 border-dashed border-white/10 rounded-2xl flex flex-col items-center justify-center gap-1 text-white/20 hover:text-teal-400 hover:border-teal-400/30 hover:bg-white/5 transition-all p-2">
             ...
             <section className="bg-white p-5 rounded-3xl border border-slate-100 shadow-lg shadow-slate-200/30">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-px bg-slate-100 border border-slate-100 rounded-2xl overflow-hidden">
                    {content.details.sections.phases?.map((phase, idx) => (
                        <div key={idx} className="p-4 bg-white hover:bg-slate-50 transition-colors relative group">
                            ...
                            <div className="flex items-center gap-3 mb-3">
                                <span className="text-[9px] font-black text-teal-500 bg-teal-50 px-2 py-1 rounded shrink-0">{phase.step}</span>
                            ...
                            <textarea placeholder="Detail" rows="3" className="w-full bg-transparent border-none outline-none text-base md:text-[10px] text-slate-400 font-medium leading-relaxed scrollbar-hide" value={phase.d} onChange={(e) => {
                                const newArr = [...content.details.sections.phases];
                    ...
                    <button onClick={() => {
                         ...
                         updateNested("details", "sections.phases", newArr);
                    }} className="bg-white flex flex-col items-center justify-center gap-1 text-slate-300 hover:text-teal-500 hover:bg-slate-50 transition-all min-h-[140px] p-2">
             ...
             <section className="bg-slate-900 p-5 rounded-3xl text-white shadow-xl">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-y-1 gap-x-8">
                     ...
                     <button onClick={() => {
                        ...
                        updateNested("details", "sections.manifest", newArr);
                     }} className="py-2 border border-dashed border-white/10 rounded-xl text-[9px] font-black uppercase text-teal-400 hover:bg-white/5 transition-all mt-2 md:mt-0 col-span-1 w-full">+ Add Protocol Node</button>
                </div>
            </section>
             ...
             <section className="bg-white p-5 rounded-3xl border border-slate-100 shadow-lg shadow-slate-200/30">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                    {content.details.sections.roi?.map((spec, idx) => (
                        <div key={idx} className="p-4 bg-slate-50 rounded-2xl relative group border border-transparent hover:border-teal-100 transition-all">
                            <button onClick={() => {
                            ...
                            <div className="flex gap-3 mb-3">
                                <div className="shrink-0 relative group/icon">
                                    <div className="w-10 h-10 rounded-xl bg-white flex items-center justify-center text-teal-600 text-xl shadow-inner border border-slate-100 group-hover:scale-105 transition-transform">
                                ...
                                <div className="flex-1 min-w-0">
                                    ...
                                    <textarea placeholder="Description" rows="2" className="w-full bg-transparent border-none outline-none text-base md:text-[10px] text-slate-400 font-medium leading-relaxed scrollbar-hide" value={spec.desc} onChange={(e) => {
                                        const newArr = [...content.details.sections.roi];
                                        newArr[idx].desc = e.target.value;
                                        updateNested("details", "sections.roi", newArr);
                                    }} />
...
function CardWrapper({ children, onRemove, highlight = false, compact = false }) {
    return (
        <div className={`p-4 bg-white border border-slate-100 rounded-2xl shadow-sm hover:shadow-lg transition-all relative group ${highlight ? 'ring-2 ring-teal-600 shadow-teal-600/10' : ''}`}>
            {onRemove && (
                <button 
                    onClick={onRemove} 
...
function AddButton({ onClick, label = "Add", small = false }) {
    if (small) return (
        <button onClick={onClick} className="p-1.5 bg-teal-50 text-teal-600 rounded-lg hover:bg-teal-600 hover:text-white transition-all">
    ...
    return (
        <button 
            onClick={onClick}
            className="h-full min-h-[80px] border-2 border-dashed border-slate-100 rounded-2xl flex flex-col items-center justify-center gap-1 text-slate-300 hover:text-teal-500 hover:border-teal-200 hover:bg-teal-50/30 transition-all group p-2"
        >
