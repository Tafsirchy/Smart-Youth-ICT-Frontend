...
import { 
  ...
  IoLayersOutline, IoSettingsOutline, IoGitNetworkOutline, IoCodeSlashOutline, 
  IoShieldCheckmarkOutline, IoHardwareChipOutline, IoPulseOutline, IoStatsChartOutline
} from "react-icons/io5";
import api from "@/lib/api";
import toast from "react-hot-toast";

const ICON_OPTIONS = ["HardwareChip", "GitNetwork", "Pulse", "Layers", "Shield", "Rocket", "Business", "Stats", "Chip", "Infinite", "Sparkles", "FingerPrint"];
...
export default function ErpCrmCMS() {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      toast.success("ERP/CRM content updated globally");
    } catch (err) {
  ...
  if (loading) return <div className="p-20 text-center font-black animate-pulse text-indigo-600">INITIALIZING ENTERPRISE ENGINE...</div>;

  return (
    <div className="p-8 max-w-7xl mx-auto pb-40 text-slate-900">
      <div className="flex justify-between items-center mb-12">
        <div>
          <h1 className="text-4xl font-black text-slate-900 mb-2 italic">ERP / CRM Systems CMS</h1>
          <p className="text-slate-500 font-bold uppercase tracking-[0.3em] text-[10px]">Operational Sovereignty Manager</p>
        </div>
        <button 
          onClick={handleSave}
          disabled={saving}
          className="px-8 py-4 bg-indigo-600 text-white rounded-xl font-black uppercase tracking-widest text-xs flex items-center gap-3 hover:bg-indigo-700 transition-all shadow-2xl shadow-indigo-600/20"
        >
      ...
      <div className="flex gap-4 mb-12 border-b border-slate-100 pb-4">
        {[
          { id: "landing", label: "Enterprise Core (Main)", icon: IoLayersOutline, color: "text-indigo-600" },
          { id: "details", label: "Operational Manifest", icon: IoHardwareChipOutline, color: "text-slate-900" },
        ].map(tab => (
          <button
            key={tab.id}
            onClick={() => setActiveTab(tab.id)}
            className={`flex items-center gap-3 px-6 py-3 rounded-xl font-black uppercase tracking-widest text-[10px] transition-all ${activeTab === tab.id ? 'bg-indigo-600 text-white shadow-xl' : 'text-slate-400 hover:text-slate-600'}`}
          >
        ...
        {activeTab === "landing" ? (
          <motion.div key="landing" initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 20 }} className="space-y-12">
             <section className="bg-white p-10 rounded-[3rem] border border-slate-100 shadow-sm">
                <h2 className="text-xl font-black uppercase tracking-tighter mb-8 flex items-center gap-3">
                    <IoHardwareChipOutline className="text-indigo-600" /> Hero Section
                </h2>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                    ...
                    <div>
                        ...
                        <input className="w-full px-5 py-3 bg-slate-50 border border-transparent focus:border-indigo-500/20 rounded-xl outline-none font-bold text-sm" value={content.landing.hero.subtitle || ""} onChange={(e) => updateNested("landing", "hero.subtitle", e.target.value)} />
                    </div>
            ...
            <section className="bg-white p-10 rounded-[3rem] border border-slate-100 shadow-sm">
                <div className="flex justify-between items-center mb-8">
                    <h2 className="text-xl font-black uppercase tracking-tighter flex items-center gap-3">
                        <IoPulseOutline className="text-indigo-600" /> Logistics Standard
                    </h2>
                    <button onClick={() => {
                        const newV = [...(content.landing.sections.verticals || []), { title: "", desc: "", icon: "HardwareChip", color: "from-indigo-600 to-slate-900" }];
                        updateNested("landing", "sections.verticals", newV);
                    }} className="text-[10px] font-black uppercase text-indigo-600 hover:text-indigo-700">+ Add Module</button>
                </div>
            ...
            <section className="bg-white p-10 rounded-[3rem] border border-slate-100 shadow-sm">
                <div className="flex justify-between items-center mb-8">
                    <h2 className="text-xl font-black uppercase tracking-tighter flex items-center gap-3">
                        <IoGitNetworkOutline className="text-indigo-600" /> System Interconnects
                    </h2>
                </div>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-px bg-slate-100 border border-slate-100 rounded-[2.5rem] overflow-hidden shadow-inner">
                    {content.landing.sections.integrations?.map((int, idx) => (
                      <div key={idx} className="p-8 bg-white hover:bg-slate-50 transition-colors">
                        <input className="w-full font-black text-[10px] text-indigo-600 uppercase tracking-widest bg-transparent outline-none mb-2" value={int.t} onChange={(e) => {
                            const newI = [...content.landing.sections.integrations];
                            newI[idx].t = e.target.value;
                            updateNested("landing", "sections.integrations", newI);
                        }} />
                        <textarea className="w-full text-[10px] text-slate-500 font-bold bg-transparent outline-none resize-none" rows="2" value={int.d} onChange={(e) => {
                            const newI = [...content.landing.sections.integrations];
          ...
          <motion.div key="details" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-12">
            ...
            <section className="bg-white p-10 rounded-[3rem] border border-slate-100 shadow-sm text-slate-900">
                <h2 className="text-xl font-black uppercase tracking-tighter mb-8 flex items-center gap-3">
                    <IoCodeSlashOutline className="text-indigo-500" /> Technical Manifest
                </h2>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                    <div>
                    ...
                    <div>
                        <label className="block text-[10px] font-black uppercase text-slate-400 mb-2 font-bold">Operational Title</label>
            ...
            <section className="bg-white p-10 rounded-[3rem] border border-slate-100 shadow-sm text-slate-900">
                <div className="flex justify-between items-center mb-8">
                    <h2 className="text-xl font-black uppercase tracking-tighter flex items-center gap-3">
                        <IoStatsChartOutline className="text-indigo-500" /> Operational ROI
                    </h2>
                    <button onClick={() => {
                        const newRoi = [...(content.details.sections.roi || []), { title: "", desc: "", icon: "Stats", features: [] }];
                        updateNested("details", "sections.roi", newRoi);
                    }} className="text-[10px] font-black uppercase text-indigo-500 hover:text-indigo-600">+ Add ROI Module</button>
                </div>
                    ...
                    {content.details.sections.roi?.map((item, idx) => (
                        <div key={idx} className="p-6 bg-slate-50 rounded-[2rem] border border-slate-100 relative group">
                            ...
                            <div className="space-y-4">
                                ...
                                <input placeholder="Module Title" className="w-full px-4 py-2 rounded-xl bg-white border border-slate-200 outline-none text-sm font-bold" value={item.title} onChange={(e) => {
                                    const newItems = [...content.details.sections.roi];
                                    newItems[idx].title = e.target.value;
                                    updateNested("details", "sections.roi", newItems);
                                }} />
                                <textarea placeholder="Technical Impact" rows="2" className="w-full px-4 py-2 rounded-xl bg-white border border-slate-200 outline-none text-xs" value={item.desc} onChange={(e) => {
                                    const newItems = [...content.details.sections.roi];
                                    newItems[idx].desc = e.target.value;
                                    updateNested("details", "sections.roi", newItems);
                                }} />
                                <div className="flex flex-wrap gap-2">
                                   ...
                                   <button onClick={() => {
                                      ...
                                      updateNested("details", "sections.roi", newRoi);
                                   }} className="text-[10px] font-black text-indigo-500">+ INDICATOR</button>
                                </div>
                            </div>
                        </div>
                    ))}
