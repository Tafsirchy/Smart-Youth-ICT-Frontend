      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-3 mb-6">
        <div>
          <div className="flex items-center gap-2 text-slate-600 mb-1">
            ...
            <span className="text-[9px] font-black uppercase tracking-[0.4em]">Service_Tier_Search_Core</span>
          </div>
          <h1 className="text-3xl font-black text-slate-900 tracking-tighter leading-none">SEO Optimization</h1>
          <p className="text-slate-400 font-bold uppercase tracking-widest text-[9px] mt-1">Centralized Visibility Node</p>
        </div>
        <button 
          onClick={handleSave}
          disabled={saving}
                            ...
                            updateNested("landing", "sections.pillars", newArr);
                             ...
                             <div className="flex gap-3 mb-3">
                                ...
                                <div className="flex-1 min-w-0">
                                    <input placeholder="Pillar Node" className="w-full bg-transparent border-none outline-none font-black text-base md:text-sm uppercase tracking-tighter" value={item.title} onChange={(e) => {
                                        const newArr = [...content.landing.sections.pillars];
                    ...
                    {content.details.sections.phases?.map((phase, idx) => (
                        <div key={idx} className="p-4 bg-slate-50 rounded-2xl relative group border border-transparent hover:border-indigo-100 transition-all">
                            ...
                            <div className="flex gap-2 mb-2">
                                <input placeholder="ID" className="w-16 bg-white border border-slate-100 rounded-lg px-2 py-1 text-base md:text-[9px] font-black text-indigo-600 uppercase" value={phase.step} onChange={(e) => {
                                    const newArr = [...content.details.sections.phases];
                                    newArr[idx].step = e.target.value;
                                    updateNested("details", "sections.phases", newArr);
                                }} />
                                <input placeholder="Stage Name" className="flex-1 bg-transparent border-none outline-none font-black text-base md:text-[10px] tracking-widest uppercase text-slate-900 focus:text-indigo-600 transition-colors" value={phase.stage} onChange={(e) => {
                                    const newArr = [...content.details.sections.phases];
                        ...
                        <div key={idx} className="p-4 bg-slate-50 rounded-3xl relative group border border-slate-100 shadow-sm">
                            ...
                            <div className="space-y-1">
                                ...
                                <button onClick={() => {
                                    ...
                                    updateNested("details", "sections.roi", newArr);
                                }} className="text-[10px] font-black text-indigo-400 uppercase tracking-widest mt-1 hover:text-indigo-600">+ Add Metric</button>
                            </div>
                        </div>
                    ))}
                </div>
