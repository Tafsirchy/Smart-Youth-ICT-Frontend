...
import { 
  ...
  IoLayersOutline, IoSettingsOutline, IoSearchOutline, IoCodeSlashOutline, 
  IoAnalyticsOutline, IoTrendingUpOutline, IoPulseOutline, IoBugOutline
} from "react-icons/io5";
import api from "@/lib/api";
import toast from "react-hot-toast";

const ICON_OPTIONS = ["Palette", "Diamond", "Flash", "Globe", "Shield", "Rocket", "Briefcase", "Stats", "Chip", "Infinite", "Sparkles", "FingerPrint", "Flask", "Images", "ShareSocial", "Shapes", "TrendingUp", "Search", "Film", "Pulse", "BarChart", "Filter", "Analytics", "Bug"];
...
export default function SeoCMS() {
  ...
  const handleSave = async () => {
    setSaving(true);
    try {
      await api.put("/cms/services/web-software/seo", content);
      toast.success("SEO content updated globally");
    } catch (err) {
  ...
  if (loading) return <div className="p-20 text-center font-black animate-pulse text-indigo-500">LOADING SEARCH ENGINE...</div>;

  return (
    <div className="p-8 max-w-7xl mx-auto pb-40">
      <div className="flex justify-between items-center mb-12">
        <div>
          <h1 className="text-4xl font-black text-slate-900 mb-2">SEO Optimization CMS</h1>
          <p className="text-slate-500 font-bold uppercase tracking-widest text-[10px]">Managing Search Authority</p>
        </div>
        <button 
          onClick={handleSave}
          disabled={saving}
      ...
      <div className="flex gap-4 mb-12 border-b border-slate-100 pb-4">
        {[
          { id: "landing", label: "Landing Architecture", icon: IoLayersOutline, color: "text-indigo-500" },
          ...
          <motion.div key="landing" initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 20 }} className="space-y-12">
            ...
            <section className="bg-white p-10 rounded-[3rem] border border-slate-100 shadow-sm">
                <h2 className="text-xl font-black uppercase tracking-tighter mb-8 flex items-center gap-3">
                    <IoSearchOutline className="text-indigo-500" /> Hero Section
                </h2>
            ...
            <section className="bg-white p-10 rounded-[3rem] border border-slate-100 shadow-sm">
                <div className="flex justify-between items-center mb-8">
                    <h2 className="text-xl font-black uppercase tracking-tighter flex items-center gap-3">
                        <IoTrendingUpOutline className="text-indigo-500" /> Search Pillars
                    </h2>
                    <button onClick={() => {
                        const newPillars = [...(content.landing.sections.pillars || []), { title: "", desc: "", icon: "TrendingUp", color: "from-indigo-500 to-slate-900" }];
                        updateNested("landing", "sections.pillars", newPillars);
                    }} className="text-[10px] font-black uppercase text-indigo-500 hover:text-indigo-600">+ Add Pillar</button>
                </div>
            ...
            <section className="bg-white p-10 rounded-[3rem] border border-slate-100 shadow-sm font-black">
                <div className="flex justify-between items-center mb-8">
                    <h2 className="text-xl font-black uppercase tracking-tighter flex items-center gap-3">
                        <IoBugOutline className="text-indigo-500" /> Technical Integrity
                    </h2>
                    <button onClick={() => {
          ...
          <motion.div key="details" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-12">
            ...
            <section className="bg-white p-10 rounded-[3rem] border border-slate-100 shadow-sm">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                    ...
                    <div>
                        <label className="block text-[10px] font-black uppercase text-slate-400 mb-2 font-bold">Visibility Title</label>
            ...
            <section className="bg-white p-10 rounded-[3rem] border border-slate-100 shadow-sm text-slate-900">
                <div className="flex justify-between items-center mb-8">
                    <h2 className="text-xl font-black uppercase tracking-tighter flex items-center gap-3">
                        <IoAnalyticsOutline className="text-indigo-500" /> Authority ROI
                    </h2>
                    <button onClick={() => {
                        const newRoi = [...(content.details.sections.roi || []), { title: "", desc: "", icon: "TrendingUp" }];
