...
import { 
  ...
  IoLayersOutline, IoSettingsOutline, IoFlashOutline, IoSyncOutline, 
  IoCheckmarkOutline, IoGitMergeOutline, IoShieldCheckmarkOutline,
  IoPulseOutline, IoStatsChartOutline, IoCloudDoneOutline, IoSparklesOutline
} from "react-icons/io5";
import api from "@/lib/api";
import toast from "react-hot-toast";

const ICON_OPTIONS = ["Flash", "Sync", "Calculator", "ShareSocial", "Layers", "Cloud", "Settings", "Lock", "Sync", "Infinite", "Stats", "GitMerge", "Pulse"];
...
export default function AutomationCMS() {
  ...
  const handleSave = async () => {
    setSaving(true);
    try {
      await api.put("/cms/services/web-software/automation", content);
      toast.success("Business Automation content updated globally");
    } catch (err) {
  ...
  if (loading) return <div className="p-20 text-center font-black animate-pulse text-amber-500 tracking-[0.3em]">SYNCHRONIZING WORKFLOW ENGINE...</div>;

  return (
    <div className="p-8 max-w-7xl mx-auto pb-40">
      <div className="flex justify-between items-center mb-12">
        <div>
          <h1 className="text-4xl font-black text-slate-900 mb-2">Business Automation CMS</h1>
          <p className="text-slate-500 font-bold uppercase tracking-widest text-[10px]">Managing Operational Velocity</p>
        </div>
        <button 
          onClick={handleSave}
          disabled={saving}
        ...
        >
          ...
          Deploy Logic Updates
        </button>
      </div>

      <div className="flex gap-4 mb-12 border-b border-slate-100 pb-4">
        {[
          { id: "landing", label: "Hero & Logic", icon: IoSyncOutline, color: "text-amber-500" },
          { id: "details", label: "Efficiency Manifest", icon: IoLayersOutline, color: "text-indigo-500" },
        ].map(tab => (
          <button
            key={tab.id}
        ...
        {activeTab === "landing" ? (
          <motion.div key="landing" initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 20 }} className="space-y-12">
            ...
            <section className="bg-white p-10 rounded-[3rem] border border-slate-100 shadow-sm">
                <h2 className="text-xl font-black uppercase tracking-tighter mb-8 flex items-center gap-3 text-amber-600">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                    <div className="md:col-span-2">
                        ...
                        <input className="w-full px-5 py-3 bg-slate-50 border border-transparent focus:border-amber-500/20 rounded-xl outline-none font-bold text-sm" value={content.landing.hero.badge || ""} onChange={(e) => updateNested("landing", "hero.badge", e.target.value)} />
                    </div>
                    <div>
                        <label className="block text-[10px] font-black uppercase text-slate-400 mb-2 font-bold tracking-widest">Primary Title</label>
                        <input className="w-full px-5 py-3 bg-slate-50 border border-transparent focus:border-amber-500/20 rounded-xl outline-none font-bold text-sm" value={content.landing.hero.title || ""} onChange={(e) => updateNested("landing", "hero.title", e.target.value)} />
                    </div>
                    <div className="md:col-span-2">
                        ...
                        <textarea rows="3" className="w-full px-5 py-3 bg-slate-50 border border-transparent focus:border-amber-500/20 rounded-xl outline-none font-medium text-sm leading-relaxed" value={content.landing.hero.description || ""} onChange={(e) => updateNested("landing", "hero.description", e.target.value)} />
                    </div>
                </div>
            </section>
            ...
            <section className="bg-white p-10 rounded-[3rem] border border-slate-100 shadow-sm">
                <div className="flex justify-between items-center mb-8">
                    <h2 className="text-xl font-black uppercase tracking-tighter flex items-center gap-3 text-amber-600">
                        <IoSyncOutline /> Automation Pillars
                    </h2>
                    <button onClick={() => {
                        const newPillars = [...(content.landing.sections.pillars || []), { title: "", desc: "", icon: "Sync", color: "from-amber-500 to-orange-600" }];
                        updateNested("landing", "sections.pillars", newPillars);
                    }} className="text-[10px] font-black uppercase text-amber-500 hover:text-amber-600">+ Add Pillar</button>
                </div>
            ...
            <section className="bg-white p-10 rounded-[3rem] border border-slate-100 shadow-sm">
                <div className="flex justify-between items-center mb-8">
                    <h2 className="text-xl font-black uppercase tracking-tighter flex items-center gap-3 text-amber-600">
                        <IoSettingsOutline /> Integration Matrix
                    </h2>
                    <button onClick={() => {
                    ...
                    }} className="text-[10px] font-black uppercase text-amber-500 hover:text-amber-600">+ Add Integration</button>
                </div>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    {content.landing.sections.integrations?.map((item, idx) => (
                        <div key={idx} className="flex items-center gap-4 bg-slate-50 p-4 rounded-2xl border border-slate-100 relative group">
                            <input placeholder="Technology/Tool" className="w-1/3 px-4 py-2 rounded-xl bg-white border border-slate-200 outline-none text-xs font-bold" value={item.t} onChange={(e) => {
                            ...
                            <input placeholder="Capability" className="flex-1 px-4 py-2 rounded-xl bg-white border border-slate-200 outline-none text-xs" value={item.d} onChange={(e) => {
                                const newItems = [...content.landing.sections.integrations];
            ...
            <section className="bg-white p-10 rounded-[3rem] border border-slate-100 shadow-sm">
                ...
                <div className="grid grid-cols-1 gap-8">
                    <div>
                        ...
                        <input className="w-full px-5 py-3 bg-slate-50 border border-transparent focus:border-amber-500/20 rounded-xl outline-none font-black text-xl tracking-tighter" value={content.landing.cta.title || ""} onChange={(e) => updateNested("landing", "cta.title", e.target.value)} />
                    </div>
                </div>
            </section>
          </motion.div>
          ...
          <motion.div key="details" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-12">
            ...
            <section className="bg-white p-10 rounded-[3rem] border border-slate-100 shadow-sm">
                <h2 className="text-xl font-black uppercase tracking-tighter mb-8 flex items-center gap-3 text-indigo-600">
                    <IoLayersOutline /> Technical Manifest
                </h2>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                    <div>
                    ...
                    <div className="md:col-span-2">
                        <label className="block text-[10px] font-black uppercase text-slate-400 mb-2 font-bold tracking-widest">Efficiency Philosophy</label>
            ...
            <section className="bg-white p-10 rounded-[3rem] border border-slate-100 shadow-sm">
                <div className="flex justify-between items-center mb-8">
                    <h2 className="text-xl font-black uppercase tracking-tighter flex items-center gap-3 text-indigo-600">
                        <IoGitMergeOutline /> Implementation Lifecycle
                    </h2>
                    <button onClick={() => {
            ...
            <section className="bg-white p-10 rounded-[3rem] border border-slate-100 shadow-sm">
                <div className="flex justify-between items-center mb-8">
                    <h2 className="text-xl font-black uppercase tracking-tighter flex items-center gap-3 text-indigo-600">
                        <IoStatsChartOutline /> Efficiency Frameworks
                    </h2>
                    <button onClick={() => {
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                    {content.details.sections.roi?.map((group, gIdx) => (
                        <div key={gIdx} className="p-8 bg-slate-50 rounded-[2.5rem] border border-slate-100 relative">
                            ...
                            <input placeholder="Framework Group (e.g. Integration Tier)" className="w-full px-4 py-2 rounded-xl bg-white border border-slate-200 outline-none text-xs font-black uppercase tracking-widest mb-6 text-indigo-600" value={group.group} onChange={(e) => {
                            ...
                            <div className="space-y-2">
                                ...
                                <button onClick={() => {
                                    ...
                                    updateNested("details", "sections.roi", newRoi);
                                }} className="text-[9px] font-black text-indigo-500 uppercase">+ Add Logic Spec</button>
                            </div>
                        </div>
                    ))}
                </div>
