...
import { 
  ...
  IoTrendingUpOutline, IoFlashOutline, IoRibbonOutline, IoSearchOutline
} from "react-icons/io5";
import api from "@/lib/api";
import toast from "react-hot-toast";

const ICON_OPTIONS = [
  "TrendingUp", "Search", "Flash", "Ribbon", "Briefcase", "Globe", "Analytics", "Rocket", "Layers"
];

const COLOR_OPTIONS = [
  { name: "Blue", value: "from-blue-600 to-emerald-600", text: "text-blue-600" },
...
export default function JobPlacementCMS() {
  ...
  const fetchData = async () => {
    try {
      // Endpoint is singular 'job-placement' per backend enum
      const res = await api.get("/cms/services/content/job-placement");
      if (res.data.data) {
        setContent(res.data.data);
      }
    } catch (err) {
      toast.error("Failed to load job placement content");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSave = async () => {
    setSaving(true);
    try {
      await api.put("/cms/services/content/job-placement", content);
      toast.success("Placement framework updated globally");
    } catch (err) {
  ...
  if (loading) return (
    <div className="min-h-screen bg-slate-50 flex flex-col items-center justify-center p-10 text-center space-y-2">
        ...
        <span className="font-black text-slate-900 uppercase tracking-widest text-xs">Initializing Career Placement Node...</span>
    </div>
  );
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-3 mb-6">
        <div>
          <div className="flex items-center gap-2 text-blue-600 mb-1">
            ...
            <span className="text-[9px] font-black uppercase tracking-[0.4em]">Service_Tier_Career_Success</span>
          </div>
          <h1 className="text-3xl font-black text-slate-900 leading-none">Job Placements</h1>
          <p className="text-slate-400 font-bold uppercase tracking-widest text-[9px] mt-1">Centralized Success Node</p>
        </div>
        <button 
          onClick={handleSave}
          disabled={saving}
            ...
            <section className="bg-white p-5 rounded-3xl border border-slate-100 shadow-lg shadow-slate-200/30">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                    ...
                    <div className="md:col-span-2">
                        <Field label="Success Manifesto" value={content.hero.description} onChange={(v) => updateNested("hero.description", v)} textarea />
                    </div>
                </div>
            </section>
             ...
             <section className="bg-slate-900 p-5 rounded-3xl text-white shadow-xl relative overflow-hidden">
                ...
                <div className="flex justify-between items-center mb-4 pb-2 border-b border-white/10">
                    <h2 className="text-xl font-black flex items-center gap-2">
                        <div className="w-8 h-8 bg-white/10 rounded-lg flex items-center justify-center text-emerald-400"><IoRocketOutline /></div>
            ...
            <section className="bg-white p-5 rounded-3xl border border-slate-100 shadow-lg shadow-slate-200/30">
                <div className="flex justify-between items-center mb-4 pb-2 border-b border-slate-100">
                    <h2 className="text-xl font-black flex items-center gap-2">
                        ...
                        Strategic Success Pillars
                    </h2>
                    <AddButton onClick={() => {
                        const newArr = [...(content.methodology || []), { title: "New Success Node", description: "" }];
                        ...
                        <div key={idx} className="p-4 bg-slate-50 rounded-3xl relative group border border-transparent hover:border-blue-100 transition-all flex flex-col h-full">
                            ...
                            <div className="flex-1 space-y-2">
                                <div>
                                    ...
                                    <input placeholder="Strategic Node" className="w-full bg-white px-2 py-1 rounded-xl border border-slate-100 outline-none font-black text-base md:text-sm uppercase text-slate-900" value={item.title} onChange={(e) => {
