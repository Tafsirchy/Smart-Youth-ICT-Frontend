...
import { 
  ...
  IoCashOutline, IoFlashOutline, IoRibbonOutline, IoGlobeOutline as IoGlobe
} from "react-icons/io5";
import api from "@/lib/api";
import toast from "react-hot-toast";

const ICON_OPTIONS = [
  "Cash", "Globe", "Flash", "Briefcase", "Analytics", "Rocket", "Layers"
];
...
export default function FreelancingCMS() {
  ...
  const fetchData = async () => {
    try {
      const res = await api.get("/cms/services/content/freelancing");
      if (res.data.data) {
        setContent(res.data.data);
      }
    } catch (err) {
      toast.error("Failed to load freelancing content");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSave = async () => {
    setSaving(true);
    try {
      await api.put("/cms/services/content/freelancing", content);
      toast.success("Global economy framework updated");
    } catch (err) {
  ...
  if (loading) return (
    <div className="min-h-screen bg-slate-50 flex flex-col items-center justify-center p-10 text-center space-y-2">
        ...
        <span className="font-black text-slate-900 uppercase tracking-widest text-xs">Initializing Global Economy Node...</span>
    </div>
  );

  return (
    <div className="min-h-screen bg-slate-50 p-4 max-w-7xl mx-auto pb-6 text-slate-900 selection:bg-amber-600 selection:text-white">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-3 mb-6">
        <div>
          <div className="flex items-center gap-2 text-amber-600 mb-1">
            <IoCashOutline className="text-lg" />
            <span className="text-[9px] font-black uppercase tracking-[0.4em]">Service_Tier_Global_Freelancing</span>
          </div>
          <h1 className="text-3xl font-black text-slate-900 leading-none">Freelancing Career</h1>
          <p className="text-slate-400 font-bold uppercase tracking-widest text-[9px] mt-1">Centralized Economy Node</p>
        </div>
        <button 
          onClick={handleSave}
          disabled={saving}
          className="hidden md:flex group relative overflow-hidden px-4 py-2 bg-slate-900 text-white rounded-xl font-black uppercase tracking-widest text-[10px] items-center gap-2 hover:shadow-xl hover:shadow-amber-950/20 transition-all active:scale-95 disabled:opacity-50"
        >
       ...
       <div className="flex gap-1 mb-6 bg-white p-1 rounded-xl border border-slate-200 w-fit shadow-sm">
        {[
          { id: "landing", label: "Economy Landing", icon: IoLayersOutline, color: "text-amber-500" },
          { id: "methodology", label: "Market Config", icon: IoSettingsOutline, color: "text-slate-500" },
        ].map(tab => (
          <button
            key={tab.id}
            ...
            <section className="bg-white p-5 rounded-3xl border border-slate-100 shadow-lg shadow-slate-200/30">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                    ...
                    <div className="md:col-span-2">
                        <Field label="Market Strategy" value={content.hero.description} onChange={(v) => updateNested("hero.description", v)} textarea />
                    </div>
                </div>
            </section>
            ...
            <section className="bg-white p-5 rounded-3xl border border-slate-100 shadow-lg shadow-slate-200/30">
                <div className="flex justify-between items-center mb-4 pb-2 border-b border-slate-100">
                    <h2 className="text-xl font-black flex items-center gap-2">
                        ...
                        Market Entry Pillars
                    </h2>
                    <AddButton onClick={() => {
                        const newArr = [...(content.methodology || []), { title: "New Market Node", description: "" }];
                        updateNested("methodology", newArr);
                    }} small />
                </div>
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
                    {content.methodology?.map((item, idx) => (
                        <div key={idx} className="p-4 bg-slate-50 rounded-3xl relative group border border-transparent hover:border-amber-100 transition-all flex flex-col h-full">
                            <button onClick={() => {
                            ...
                            <div className="w-10 h-10 rounded-xl bg-amber-600 text-white flex items-center justify-center font-black text-sm mb-3 shadow-lg shadow-amber-600/20">
                                {String(idx + 1).padStart(2, '0')}
                            </div>
                            
                            <div className="flex-1 space-y-2">
                                <div>
                                    <label className="text-[8px] font-black uppercase text-slate-400 tracking-widest mb-1 block">Market_Node</label>
                                    <input placeholder="Strategic Entry" className="w-full bg-white px-2 py-1 rounded-xl border border-slate-100 outline-none font-black text-base md:text-sm uppercase text-slate-900" value={item.title} onChange={(e) => {
                                ...
                                <div>
                                    <label className="text-[8px] font-black uppercase text-slate-400 tracking-widest mb-1 block">Operational_Process</label>
                                    <textarea placeholder="Scientific market entry" rows="4" className="w-full bg-white px-2 py-1.5 rounded-xl border border-slate-100 outline-none text-base md:text-[11px] text-slate-500 font-medium leading-normal resize-none" value={item.description} onChange={(e) => {
                                        const newArr = [...content.methodology];
                    ...
                    {content.methodology?.length === 0 && <div className="md:col-span-3 py-10 border-2 border-dashed border-slate-100 rounded-3xl flex flex-col items-center justify-center gap-2 text-slate-300">
                        ...
                        <p className="text-[10px] font-black uppercase tracking-[0.3em]">Initialize Market Entry architecture</p>
                    </div>}
                </div>
            </section>
      ...
      <div className="md:hidden sticky bottom-0 left-0 w-full pt-4 pb-[calc(1rem+env(safe-area-inset-bottom))] bg-slate-50/90 backdrop-blur-md z-[90]">
        ...
        >
          <div className="absolute inset-0 bg-gradient-to-r from-amber-600 to-slate-900 opacity-0 hover:opacity-100 transition-opacity" />
          <div className="relative flex items-center gap-2">
...
function AddButton({ onClick, label = "Add", small = false }) {
    if (small) return (
        <button onClick={onClick} className="p-1 bg-amber-50 text-amber-600 rounded-lg hover:bg-amber-600 hover:text-white transition-all">
    ...
    return (
        <button 
            onClick={onClick}
            className="h-full min-h-[80px] border-2 border-dashed border-slate-100 rounded-2xl flex flex-col items-center justify-center gap-1 text-slate-300 hover:text-amber-500 hover:border-amber-200 hover:bg-amber-50/30 transition-all group"
        >
