      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-3 mb-6">
        <div>
          <div className="flex items-center gap-2 text-blue-600 mb-1">
            ...
            <span className="text-[9px] font-black uppercase tracking-[0.4em]">Service_Tier_Infrastructure</span>
          </div>
          <h1 className="text-3xl font-black text-slate-900 tracking-tighter leading-none">Cloud Hosting & Security</h1>
          <p className="text-slate-400 font-bold uppercase tracking-widest text-[9px] mt-1 ml-1">Centralized Cloud Node</p>
        </div>
        <button 
          onClick={handleSave}
          disabled={saving}
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
                    {content.landing.sections.pillars?.map((item, idx) => (
                        <CardWrapper key={idx} onRemove={() => {
                            ...
                            updateNested("landing", "sections.pillars", newArr);
                             ...
                             <div className="flex gap-3 mb-3">
                                <div className="shrink-0 relative group/icon">
                                    <div className="w-10 h-10 rounded-xl bg-slate-50 flex items-center justify-center text-blue-600 text-xl shadow-inner border border-slate-100 group-hover:scale-105 transition-all">
                            ...
                            <textarea placeholder="Technical intent" rows="3" className="w-full bg-slate-50 border border-transparent focus:bg-white focus:border-slate-100 rounded-xl p-2 text-base md:text-[11px] text-slate-500 leading-relaxed outline-none transition-all scrollbar-hide" value={item.desc} onChange={(e) => {
                                const newArr = [...content.landing.sections.pillars];
            ...
            <section className="bg-slate-900 p-5 rounded-3xl text-white shadow-xl">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                    ...
                    <button onClick={() => {
                        ...
                        updateNested("landing", "sections.metrics", newArr);
                    }} className="border-2 border-dashed border-white/10 rounded-xl flex items-center justify-center py-4 text-white/20 hover:text-blue-400 hover:bg-white/5 transition-all">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-3">
                    {content.landing.sections.plans?.map((plan, idx) => (
                        <CardWrapper key={idx} onRemove={() => {
                            ...
                            <div className="grid grid-cols-2 gap-3 mb-3 p-3 bg-slate-50 rounded-xl border border-slate-100 pt-3">
                            ...
                            <div>
                                <label className="block text-[10px] font-black uppercase mb-1 tracking-widest text-slate-400">Features Array (Comma Separated)</label>
                                <textarea rows="3" className="w-full bg-slate-50 border border-transparent focus:bg-white focus:border-slate-100 rounded-xl p-2 text-base md:text-[11px] text-slate-600 leading-relaxed outline-none transition-all scrollbar-hide" value={(plan.features || []).join(", ")} onChange={(e) => {
                                    const newArr = [...content.landing.sections.plans];
                    ...
                    {content.details.sections.phases?.map((phase, idx) => (
                        <CardWrapper key={idx} onRemove={() => {
                            ...
                            updateNested("details", "sections.phases", newArr);
                             ...
                             <textarea placeholder="Action logic..." rows="3" className="w-full bg-slate-50 rounded-xl p-2 text-base md:text-[10px] text-slate-500 font-medium outline-none scrollbar-hide border border-transparent focus:border-blue-100 transition-colors" value={phase.action} onChange={(e) => {
                                const newArr = [...content.details.sections.phases];
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                    {content.details.sections.roi?.map((fw, idx) => (
                        <div key={idx} className="bg-white/5 border border-white/10 p-4 rounded-2xl relative group">
                            ...
                            <textarea placeholder="Items (Comma separated)" rows="3" className="w-full bg-transparent border border-white/5 focus:border-white/20 rounded-xl p-2 text-base md:text-[11px] text-white/70 font-medium leading-relaxed outline-none scrollbar-hide transition-colors" value={(fw.items || []).join(', ')} onChange={(e) => {
                                const newArr = [...content.details.sections.roi];
...
function AddButton({ onClick, label = "Add", small = false }) {
    if (small) return (
        <button onClick={onClick} className="p-1.5 bg-blue-50 text-blue-600 rounded-lg hover:bg-blue-600 hover:text-white transition-all">
    ...
    return (
        <button 
            onClick={onClick}
            className="h-full min-h-[80px] border-2 border-dashed border-slate-100 rounded-2xl flex flex-col items-center justify-center gap-1 text-slate-300 hover:text-blue-500 hover:border-blue-200 hover:bg-blue-50/30 transition-all group p-2"
        >
