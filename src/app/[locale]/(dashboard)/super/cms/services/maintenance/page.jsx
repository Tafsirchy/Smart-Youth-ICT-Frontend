...
import { 
  ...
  IoBuildOutline, IoFlashOutline, IoShieldCheckmarkOutline as IoShieldC
} from "react-icons/io5";
import api from "@/lib/api";
import toast from "react-hot-toast";

const ICON_OPTIONS = [
  "Build", "Shield", "Flash", "Globe", "Analytics", "Rocket", "Layers"
];
...
export default function MaintenanceCMS() {
  ...
  const fetchData = async () => {
    try {
      const res = await api.get("/cms/services/ai-managed/maintenance");
      if (res.data.data) {
        setContent(prev => ({
            ...prev,
            ...res.data.data,
  ...
  const handleSave = async () => {
    setSaving(true);
    try {
      await api.put("/cms/services/ai-managed/maintenance", content);
      toast.success("Support framework updated globally");
    } catch (err) {
  ...
  if (loading) return (
    <div className="min-h-screen bg-slate-50 flex flex-col items-center justify-center p-10 text-center space-y-2">
        ...
        <span className="font-black text-slate-900 uppercase tracking-widest text-xs">Initializing Support Node...</span>
    </div>
  );

  return (
    <div className="min-h-screen bg-slate-50 p-4 max-w-7xl mx-auto pb-6 text-slate-900 selection:bg-slate-900 selection:text-white">
      {/* HEADER PROTOCOL */}
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-3 mb-6">
        <div>
          <div className="flex items-center gap-2 text-slate-600 mb-1">
            <IoBuildOutline className="text-lg" />
            <span className="text-[9px] font-black uppercase tracking-[0.4em]">Service_Tier_Support</span>
          </div>
          <h1 className="text-3xl font-black text-slate-900 tracking-tighter leading-none">Maintenance & Support</h1>
          <p className="text-slate-400 font-bold uppercase tracking-widest text-[9px] mt-1">Centralized Maintenance Node</p>
        </div>
        <button 
          onClick={handleSave}
          disabled={saving}
          className="hidden md:flex group relative overflow-hidden px-4 py-2 bg-slate-900 text-white rounded-xl font-black uppercase tracking-widest text-[10px] items-center gap-2 hover:shadow-xl hover:shadow-slate-950/20 transition-all active:scale-95 disabled:opacity-50"
        >
       ...
       <div className="flex gap-1 mb-6 bg-white p-1 rounded-xl border border-slate-200 w-fit shadow-sm">
        {[
          { id: "landing", label: "Support Landing", icon: IoLayersOutline, color: "text-slate-500" },
          ...
          >
            <tab.icon size={16} className={activeTab === tab.id ? 'text-slate-400' : tab.color} />
            {tab.label}
          </button>
        ))}
      </div>
          ...
          <motion.div key="landing" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="space-y-6">
            ...
            <section className="bg-white p-5 rounded-3xl border border-slate-100 shadow-lg shadow-slate-200/30">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                    ...
                    <div className="md:col-span-2">
                        <Field label="Support Strategy" value={content.landing.hero.description} onChange={(v) => updateNested("landing", "hero.description", v)} textarea />
                    </div>
                </div>
            </section>
             ...
             <section className="bg-white p-5 rounded-3xl border border-slate-100 shadow-lg shadow-slate-200/30">
                <div className="flex justify-between items-center mb-4 pb-2 border-b border-slate-100">
                    <h2 className="text-xl font-black tracking-tighter flex items-center gap-2">
                        <div className="w-8 h-8 bg-slate-50 rounded-lg flex items-center justify-center text-slate-600"><IoFlashOutline /></div>
                        Maintenance Pillars
                    </h2>
                </div>
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
                    {content.landing.sections.pillars?.map((item, idx) => (
                        <CardWrapper key={idx} onRemove={() => {
                            ...
                            updateNested("landing", "sections.pillars", newArr);
                             ...
                             <div className="flex gap-3 mb-3">
                                <div className="shrink-0 relative group/icon">
                                    <div className="w-10 h-10 rounded-xl bg-slate-50 flex items-center justify-center text-slate-900 text-xl shadow-inner border border-slate-100 group-hover:scale-105 transition-all">
                                        {item.icon === "Build" && <IoBuildOutline />}
                            ...
                            <textarea placeholder="Technical rationale" rows="3" className="w-full bg-slate-50 border border-transparent focus:bg-white focus:border-slate-100 rounded-xl p-2 text-base md:text-[11px] text-slate-500 leading-relaxed outline-none transition-all scrollbar-hide" value={item.desc} onChange={(e) => {
                                const newArr = [...content.landing.sections.pillars];
                    ...
                    <AddButton onClick={() => {
                        const newArr = [...(content.landing.sections.pillars || []), { title: "New Maintenance Node", desc: "", icon: "Shield", color: "from-slate-700 to-slate-900" }];
                        updateNested("landing", "sections.pillars", newArr);
                    }} label="Add Maintenance Node" />
                </div>
            </section>
            ...
            <section className="bg-slate-900 p-5 rounded-3xl text-white shadow-xl">
                 <div className="flex justify-between items-center mb-4 pb-2 border-b border-white/10">
                    <h2 className="text-xl font-black tracking-tighter flex items-center gap-2">
                        <div className="w-8 h-8 bg-white/10 rounded-lg flex items-center justify-center text-slate-400"><IoStatsChartOutline /></div>
                        Uptime Metrics
                    </h2>
                </div>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                    ...
                    <button onClick={() => {
                        ...
                        updateNested("landing", "sections.metrics", newArr);
                    }} className="border-2 border-dashed border-white/10 rounded-xl flex items-center justify-center py-4 text-white/20 hover:text-teal-400 hover:bg-white/5 transition-all">
            ...
            <section className="bg-white p-5 rounded-3xl border border-slate-100 shadow-lg shadow-slate-200/30">
                <div className="flex justify-between items-center mb-4 pb-2 border-b border-slate-100">
                    <h2 className="text-xl font-black tracking-tighter flex items-center gap-2">
                        <div className="w-8 h-8 bg-slate-50 rounded-lg flex items-center justify-center text-slate-600"><IoRocketOutline /></div>
                ...
                <div className="max-w-md">
                    <Field 
                        ...
                        icon={<IoFlashOutline className="text-slate-600" />}
                    />
                </div>
            </section>
          ...
          <motion.div key="details" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="space-y-6">
            ...
            <section className="bg-white p-5 rounded-3xl border border-slate-100 shadow-lg shadow-slate-200/30">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                    ...
                    <div className="md:col-span-2">
                        <Field label="Technical Summary" value={content.details.hero.description} onChange={(v) => updateNested("details", "hero.description", v)} textarea />
                    </div>
                </div>
            </section>

            {/* MAINTENANCE PROTOCOL (PHASES) */}
            <section className="bg-white p-5 rounded-3xl border border-slate-100 shadow-lg shadow-slate-200/30">
                <div className="flex justify-between items-center mb-4 pb-2 border-b border-slate-100">
                    <h2 className="text-xl font-black tracking-tighter flex items-center gap-2">
                    ...
                    <AddButton onClick={() => {
                        const newArr = [...(content.details.sections.phases || []), { step: "STEP_01", stage: "", action: "" }];
                    ...
                    {content.details.sections.phases?.map((phase, idx) => (
                        <CardWrapper key={idx} onRemove={() => {
                            ...
                            <div className="flex gap-2 mb-2">
                                <input placeholder="STEP_01" className="w-16 bg-slate-50 border-none outline-none font-mono text-base md:text-[9px] text-amber-600 font-bold px-2 py-1 rounded-md uppercase" value={phase.step} onChange={(e) => {
                            ...
                            <textarea placeholder="Phase action/description" rows="3" className="w-full bg-slate-50 border border-transparent focus:bg-white focus:border-slate-100 rounded-xl p-2 text-base md:text-[10px] text-slate-500 leading-relaxed outline-none transition-all scrollbar-hide" value={phase.action} onChange={(e) => {
                                    const newArr = [...content.details.sections.phases];
            ...
            <section className="bg-white p-5 rounded-3xl border border-slate-100 shadow-lg shadow-slate-200/30">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                    {content.details.sections.roi?.map((spec, idx) => (
                        <div key={idx} className="p-4 bg-slate-50 rounded-2xl relative group border border-transparent hover:border-slate-200 transition-all">
                            <button onClick={() => {
                            ...
                            <div className="mb-3">
                                ...
                                <input placeholder="e.g. Infrastructure Tier" className="w-full bg-transparent border-none outline-none font-black text-base md:text-sm tracking-tighter uppercase text-slate-900 focus:text-slate-600 transition-colors" value={spec.group} onChange={(e) => {
                            ...
                            <div>
                                ...
                                <textarea 
                                    placeholder="Sentry Audit, SSL Sync, Node Patch..." 
                                    rows="3" 
                                    className="w-full bg-white border border-slate-100 rounded-xl p-2 text-base md:text-[10px] text-slate-500 font-medium leading-relaxed outline-none transition-all scrollbar-hide" 
      ...
      <div className="md:hidden sticky bottom-0 left-0 w-full pt-4 pb-[calc(1rem+env(safe-area-inset-bottom))] bg-slate-50/90 backdrop-blur-md z-[90]">
        ...
        >
          <div className="absolute inset-0 bg-gradient-to-r from-slate-700 to-slate-900 opacity-0 hover:opacity-100 transition-opacity" />
          <div className="relative flex items-center gap-2">
...
function Field({ label, value, onChange, textarea = false, dark = false, small = false, icon = null }) {
    return (
        <div className={small ? "w-48" : "w-full"}>
            ...
            <div className={`relative flex items-center ${dark ? 'bg-white/5 border-white/5' : 'bg-slate-50 border-transparent'} border rounded-xl overflow-hidden focus-within:border-slate-500/10 transition-all`}>
...
function CardWrapper({ children, onRemove, highlight = false, compact = false }) {
    return (
        <div className={`p-4 bg-white border border-slate-100 rounded-2xl shadow-sm hover:shadow-lg transition-all relative group ${highlight ? 'ring-2 ring-slate-900 shadow-slate-900/10' : ''}`}>
            {onRemove && (
                <button 
                    onClick={onRemove} 
...
function AddButton({ onClick, label = "Add", small = false }) {
    if (small) return (
        <button onClick={onClick} className="p-1.5 bg-slate-50 text-slate-900 rounded-lg hover:bg-slate-900 hover:text-white transition-all">
    ...
    return (
        <button 
            onClick={onClick}
            className="h-full min-h-[80px] border-2 border-dashed border-slate-100 rounded-2xl flex flex-col items-center justify-center gap-1 text-slate-300 hover:text-slate-900 hover:border-slate-200 hover:bg-slate-50/30 transition-all group p-2"
        >
