...
import { 
  ...
  IoLayersOutline, IoSettingsOutline, IoImagesOutline, IoCodeSlashOutline, 
  IoSparklesOutline, IoFilmOutline, IoPulseOutline, IoShareSocialOutline
} from "react-icons/io5";
import api from "@/lib/api";
import toast from "react-hot-toast";

const ICON_OPTIONS = ["Palette", "Diamond", "Flash", "Globe", "Shield", "Rocket", "Briefcase", "Stats", "Chip", "Infinite", "Sparkles", "FingerPrint", "Flask", "Images", "ShareSocial", "Shapes", "TrendingUp", "Search", "Film", "Pulse"];
...
export default function SocialCreativesCMS() {
  ...
  const handleSave = async () => {
    setSaving(true);
    try {
      await api.put("/cms/services/web-software/social-creatives", content);
      toast.success("Social Creatives content updated globally");
    } catch (err) {
  ...
  if (loading) return <div className="p-20 text-center font-black animate-pulse text-rose-400">LOADING CREATIVE ENGINE...</div>;

  return (
    <div className="p-8 max-w-7xl mx-auto pb-40">
      <div className="flex justify-between items-center mb-12">
        <div>
          <h1 className="text-4xl font-black text-slate-900 mb-2">Social Media Creatives CMS</h1>
          <p className="text-slate-500 font-bold uppercase tracking-widest text-[10px]">Managing Attention Capital</p>
        </div>
        <button 
          onClick={handleSave}
          disabled={saving}
          ...
          <motion.div key="landing" initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 20 }} className="space-y-12">
            ...
            <section className="bg-white p-10 rounded-[3rem] border border-slate-100 shadow-sm">
                <h2 className="text-xl font-black uppercase tracking-tighter mb-8 flex items-center gap-3">
                    <IoImagesOutline className="text-rose-500" /> Hero Section
                </h2>
            ...
            <section className="bg-white p-10 rounded-[3rem] border border-slate-100 shadow-sm">
                <div className="flex justify-between items-center mb-8">
                    <h2 className="text-xl font-black uppercase tracking-tighter flex items-center gap-3">
                        <IoSparklesOutline className="text-rose-500" /> Creative Pillars
                    </h2>
                    <button onClick={() => {
                        const newPillars = [...(content.landing.sections.pillars || []), { title: "", desc: "", icon: "Images", color: "from-rose-500 to-amber-600" }];
            ...
            <section className="bg-white p-10 rounded-[3rem] border border-slate-100 shadow-sm font-black">
                <div className="flex justify-between items-center mb-8">
                    <h2 className="text-xl font-black uppercase tracking-tighter flex items-center gap-3">
                        <IoFilmOutline className="text-rose-500" /> Interaction Metrics
                    </h2>
                    <button onClick={() => {
                        const newMetrics = [...(content.landing.sections.metrics || []), { t: "" }];
                        updateNested("landing", "sections.metrics", newMetrics);
                    }} className="text-[10px] font-black uppercase text-rose-500 hover:text-rose-600">+ Add Metric</button>
                </div>
          ...
          <motion.div key="details" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-12">
            ...
            <section className="bg-white p-10 rounded-[3rem] border border-slate-100 shadow-sm">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                    ...
                    <div>
                        <label className="block text-[10px] font-black uppercase text-slate-400 mb-2 font-bold">Creative Title</label>
            ...
            <section className="bg-white p-10 rounded-[3rem] border border-slate-100 shadow-sm text-slate-900">
                <div className="flex justify-between items-center mb-8">
                    <h2 className="text-xl font-black uppercase tracking-tighter flex items-center gap-3">
                        <IoPulseOutline className="text-indigo-500" /> Engagement ROI
                    </h2>
                    <button onClick={() => {
                        const newRoi = [...(content.details.sections.roi || []), { title: "", desc: "", icon: "ShareSocial" }];
