...
import { 
  ...
  IoFilmOutline, IoImagesOutline, IoFlashOutline, IoTrendingUpOutline
} from "react-icons/io5";
import api from "@/lib/api";
import toast from "react-hot-toast";

const ICON_OPTIONS = [
  "Images", "Film", "Flash", "TrendingUp", "Palette", "Globe", "Analytics", "Stats", "Rocket", "Layers"
];

const COLOR_OPTIONS = [
  { name: "Rose", value: "from-rose-500 to-pink-600", text: "text-rose-500" },
  { name: "Pink", value: "from-pink-600 to-amber-600", text: "text-pink-600" },
  { name: "Amber", value: "from-amber-500 to-rose-600", text: "text-amber-500" },
...
export default function SocialCreativesCMS() {
  ...
  const fetchData = async () => {
    try {
      const res = await api.get("/cms/services/creative-marketing/social-creatives");
      if (res.data.data) {
        setContent(prev => ({
            ...prev,
            ...res.data.data,
      ...
      toast.error("Failed to load social creatives content");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSave = async () => {
    setSaving(true);
    try {
      await api.put("/cms/services/creative-marketing/social-creatives", content);
      toast.success("Engagement engine updated globally");
    } catch (err) {
  ...
  if (loading) return (
    <div className="min-h-screen bg-slate-50 flex flex-col items-center justify-center p-10 text-center space-y-2">
        ...
        <span className="font-black text-slate-900 uppercase tracking-widest text-xs">Initializing Creative Content Lab...</span>
    </div>
  );
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-3 mb-6">
        <div>
          <div className="flex items-center gap-2 text-rose-600 mb-1">
            ...
            <span className="text-[9px] font-black uppercase tracking-[0.4em]">Service_Tier_Scroll_Stoppers</span>
          </div>
          <h1 className="text-3xl font-black text-slate-900 tracking-tighter leading-none">Social Media Creatives</h1>
          <p className="text-slate-400 font-bold uppercase tracking-widest text-[9px] mt-1">Centralized Engagement Node</p>
        </div>
        <button 
          onClick={handleSave}
          disabled={saving}
       ...
       <div className="flex gap-1 mb-6 bg-white p-1 rounded-xl border border-slate-200 w-fit shadow-sm">
        {[
          { id: "landing", label: "Creative Landing", icon: IoLayersOutline, color: "text-rose-500" },
          { id: "details", label: "Content Manifest", icon: IoSettingsOutline, color: "text-indigo-500" },
        ].map(tab => (
          <button
            key={tab.id}
        ...
        {activeTab === "landing" ? (
          <motion.div key="landing" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="space-y-6">
            ...
            <section className="bg-white p-5 rounded-3xl border border-slate-100 shadow-lg shadow-slate-200/30">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                    ...
                    <div className="md:col-span-2">
                        <Field label="Creative Strategy" value={content.landing.hero.description} onChange={(v) => updateNested("landing", "hero.description", v)} textarea />
                    </div>
                </div>
            </section>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
                    {content.landing.sections.pillars?.map((item, idx) => (
                        <CardWrapper key={idx} onRemove={() => {
                            ...
                            updateNested("landing", "sections.pillars", newArr);
                             ...
                             <div className="flex gap-3 mb-3">
                                <div className="shrink-0 relative group/icon">
                                    <div className="w-10 h-10 rounded-xl bg-slate-50 flex items-center justify-center text-rose-600 text-xl shadow-inner border border-slate-100 group-hover:scale-105 transition-all">
                                        {item.icon === "Images" && <IoImagesOutline />}
                                        {item.icon === "Film" && <IoFilmOutline />}
                                ...
                                <div className="flex-1 min-w-0">
                                    <input placeholder="Hook Type" className="w-full bg-transparent border-none outline-none font-black text-base md:text-sm uppercase tracking-tighter" value={item.title} onChange={(e) => {
                                        const newArr = [...content.landing.sections.pillars];
                            ...
                            <textarea placeholder="Psychological hook" rows="3" className="w-full bg-slate-50 border border-transparent focus:bg-white focus:border-slate-100 rounded-xl p-2 text-base md:text-[11px] text-slate-500 leading-relaxed outline-none transition-all scrollbar-hide" value={item.desc} onChange={(e) => {
                                const newArr = [...content.landing.sections.pillars];
                    ...
                    <AddButton onClick={() => {
                        const newArr = [...(content.landing.sections.pillars || []), { title: "New Hook", desc: "", icon: "Flash", color: "from-rose-500 to-pink-600" }];
                        updateNested("landing", "sections.pillars", newArr);
                    }} label="Add Hook Pillar" />
                </div>
            </section>
            ...
            <section className="bg-slate-900 p-5 rounded-3xl text-white shadow-xl">
                 <div className="flex justify-between items-center mb-4 pb-2 border-b border-white/10">
                    <h2 className="text-xl font-black tracking-tighter flex items-center gap-2">
                        <div className="w-8 h-8 bg-white/10 rounded-lg flex items-center justify-center text-rose-400"><IoStatsChartOutline /></div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
                    ...
                    <button onClick={() => {
                        const newArr = [...(content.landing.sections.metrics || []), { t: "NEW_SUITE" }];
                        updateNested("landing", "sections.metrics", newArr);
                    }} className="border-2 border-dashed border-white/10 rounded-xl flex items-center justify-center py-2 text-white/20 hover:text-rose-400 hover:bg-white/5 transition-all">
          ...
          <motion.div key="details" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="space-y-6">
            ...
            <section className="bg-white p-5 rounded-3xl border border-slate-100 shadow-lg shadow-slate-200/30">
                <div className="flex justify-between items-center mb-4 pb-2 border-b border-slate-100">
                    <h2 className="text-xl font-black tracking-tighter flex items-center gap-2">
                        <div className="w-8 h-8 bg-rose-50 rounded-lg flex items-center justify-center text-rose-600"><IoGitNetworkOutline /></div>
                        The Creative Protocol (Lifecycle)
                    </h2>
                    <AddButton onClick={() => {
                        const newArr = [...(content.details.sections.phases || []), { step: "CREATE_0X", stage: "", action: "" }];
                    ...
                    {content.details.sections.phases?.map((phase, idx) => (
                        <div key={idx} className="p-4 bg-slate-50 rounded-2xl relative group border border-transparent hover:border-rose-100 transition-all">
                            ...
                            <div className="flex gap-2 mb-2">
                                <input placeholder="ID" className="w-16 bg-white border border-slate-100 rounded-lg px-2 py-1 text-base md:text-[9px] font-black text-rose-600 uppercase" value={phase.step} onChange={(e) => {
                                    const newArr = [...content.details.sections.phases];
                                    newArr[idx].step = e.target.value;
                                    updateNested("details", "sections.phases", newArr);
                                }} />
                                <input placeholder="Stage Name" className="flex-1 bg-transparent border-none outline-none font-black text-base md:text-[10px] tracking-widest uppercase text-slate-900 focus:text-rose-600 transition-colors" value={phase.stage} onChange={(e) => {
                                    const newArr = [...content.details.sections.phases];
                        ...
                        <div key={idx} className="p-4 bg-slate-50 rounded-3xl relative group border border-slate-100 shadow-sm">
                            ...
                            <div className="space-y-1">
                                ...
                                <button onClick={() => {
                                    ...
                                    updateNested("details", "sections.roi", newArr);
                                }} className="text-[10px] font-black text-rose-400 uppercase tracking-widest mt-1 hover:text-rose-600">+ Add Objective</button>
                            </div>
                        </div>
                    ))}
                </div>
