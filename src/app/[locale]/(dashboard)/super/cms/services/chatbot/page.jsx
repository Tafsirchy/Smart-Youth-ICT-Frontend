  ...
  return (
    <div className="min-h-screen bg-slate-50 p-4 max-w-7xl mx-auto pb-6 text-slate-900 selection:bg-indigo-600 selection:text-white">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-3 mb-6">
        <div>
          <div className="flex items-center gap-2 text-indigo-600 mb-1">
            ...
            <span className="text-[9px] font-black uppercase tracking-[0.4em]">Service_Tier_Neural_Agents</span>
          </div>
          <h1 className="text-3xl font-black text-slate-900 tracking-tighter leading-none">AI Chatbot Solutions</h1>
          <p className="text-slate-400 font-bold uppercase tracking-widest text-[9px] mt-1">Centralized Intelligent Node</p>
        </div>
        <div className="flex items-center gap-2 w-full md:w-auto">
             <div className="hidden lg:flex items-center gap-2 bg-white border border-slate-200 px-3 py-1.5 rounded-xl shadow-sm">
                ...
                <div className="flex flex-col">
                    ...
                    <span className={`text-[9px] font-black uppercase ${integrity.percentage > 70 ? 'text-emerald-600' : 'text-amber-600'}`}>
                        {integrity.percentage === 100 ? 'Production Ready' : 'Optimization Needed'}
                    </span>
                </div>
                {integrity.missing.length > 0 && (
                    <div className="group/missing relative ml-1">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
                    {content.landing.sections.pillars?.map((item, idx) => (
                        <CardWrapper key={idx} onRemove={() => {
                            ...
                            <textarea placeholder="Neural rationale" rows="3" className="w-full bg-slate-50 border border-transparent focus:bg-white focus:border-slate-100 rounded-xl p-2 text-base md:text-[11px] text-slate-500 leading-relaxed outline-none transition-all scrollbar-hide" value={item.desc} onChange={(e) => {
                                const newArr = [...content.landing.sections.pillars];
            ...
            <section className="bg-slate-900 p-5 rounded-3xl text-white shadow-xl">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-3">
                    ...
                    <button onClick={() => {
                        ...
                        updateNested("landing", "sections.metrics", newArr);
                    }} className="border-2 border-dashed border-white/10 rounded-xl flex items-center justify-center py-2 text-white/20 hover:text-indigo-400 hover:bg-white/5 transition-all">
                    ...
                    {content.details.sections.roi?.map((spec, idx) => (
                        <div key={idx} className="p-4 bg-slate-50 rounded-2xl relative group border border-transparent hover:border-indigo-100 transition-all">
                            ...
                            <div className="mb-3">
                                ...
                                <input placeholder="e.g. Intelligence Tier" className="w-full bg-transparent border-none outline-none font-black text-base md:text-sm tracking-tighter uppercase text-slate-900 focus:text-indigo-600 transition-colors" value={spec.group} onChange={(e) => {
      ...
      <div className="md:hidden sticky bottom-0 left-0 w-full pt-4 pb-[calc(1rem+env(safe-area-inset-bottom))] bg-slate-50/90 backdrop-blur-md z-[90]">
        ...
        >
          <div className="absolute inset-0 bg-gradient-to-r from-indigo-600 to-slate-900 opacity-0 hover:opacity-100 transition-opacity" />
          <div className="relative flex items-center gap-2">
    ...
    return (
        <div className={small ? "w-48" : "w-full"}>
            <div className="flex justify-between items-center mb-1">
                <label className={`block text-[9px] font-black uppercase tracking-widest ${dark ? 'text-slate-500' : 'text-slate-400'}`}>
                    {label}
                </label>
