      ...
      <div className="container-custom py-20 relative">
        ...
        <div className="flex flex-col lg:flex-row items-center gap-10 lg:gap-20 mb-20 lg:mb-48">
          <div className="flex-1 text-left">
            <motion.div
              ...
              className="inline-flex items-center gap-2 px-5 py-2 rounded-full bg-indigo-50 border border-indigo-100 text-indigo-700 text-[10px] sm:text-xs font-black tracking-[0.2em] sm:tracking-[0.4em] uppercase mb-10"
            >
          ...
          <div className="flex-1 relative hidden lg:block">
            ...
            >
              ...
              <div className="relative aspect-square flex items-center justify-center">
                ...
                >
                  ...
                  <div className="relative z-10 text-center">
                    ...
                    <p className="text-[10px] sm:text-xs font-black text-slate-400 tracking-widest uppercase mb-1">Crawl Status</p>
                    <p className="text-xs sm:text-sm font-mono text-emerald-500 font-bold">200_OK_VERIFIED</p>
                  </div>
                </motion.div>
              ...
              <div className="absolute bottom-8 left-1/2 -translate-x-1/2 font-mono text-[8px] sm:text-[10px] text-slate-300 uppercase tracking-[0.2em] sm:tracking-[0.5em] bg-white px-4">
        ...
        <div className="mb-20 lg:mb-48">
          <div className="grid lg:grid-cols-2 gap-10 lg:gap-20 items-center bg-white rounded-3xl lg:rounded-[4rem] p-8 sm:p-12 lg:p-24 border border-slate-100 shadow-xl lg:shadow-2xl shadow-slate-200/50 relative overflow-hidden">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-px bg-slate-50 border border-slate-100 rounded-3xl overflow-hidden mt-8 sm:mt-10 shadow-sm">
                {metrics?.map((item, idx) => (
                  <div key={idx} className="p-6 sm:p-10 hover:bg-white transition-colors group">
                    ...
                    <h4 className="text-[10px] sm:text-xs font-black text-indigo-600 uppercase tracking-widest mb-2">{item.t}</h4>
                    <p className="text-[10px] sm:text-xs text-slate-400 font-bold leading-tight line-clamp-2">{item.d}</p>
                  </div>
                ))}
              </div>
            </div>
            ...
            >
              <div className="bg-slate-900 rounded-3xl lg:rounded-[3rem] p-8 lg:p-12 border border-slate-800 shadow-2xl space-y-8 lg:space-y-10 group overflow-hidden">
                ...
                <div className="pt-8 border-t border-white/5">
                  <p className="text-[10px] sm:text-xs font-mono text-emerald-400 font-bold tracking-tighter">SUCCESS_INDEX::94/100</p>
                  <p className="text-[10px] sm:text-xs font-mono text-slate-500 uppercase tracking-widest mt-2 flex items-center gap-2">
        ...
        <div className="text-center py-20 lg:py-40 border-t border-slate-200 pb-32 lg:pb-40">
          <IoAnalyticsOutline className="text-6xl lg:text-7xl text-indigo-600 mb-8 lg:mb-12 mx-auto opacity-20" />
          <h3 className="text-4xl sm:text-5xl lg:text-6xl font-black text-slate-900 mb-10 lg:mb-12 leading-tight tracking-tighter sm:tracking-normal">
            {cta.title?.includes('.') ? (
              <>
                ...
                <span className="text-transparent bg-clip-text bg-gradient-to-r from-indigo-600 to-slate-700 font-serif italic font-medium block sm:inline mt-2 sm:mt-0">
                  {cta.title.split('.').slice(1).join('.').trim()}
                </span>
              </>
            ) : (
              <>
                Stop playing catch up. <br className="hidden sm:block" />
                <span className="text-transparent bg-clip-text bg-gradient-to-r from-indigo-600 to-slate-700 font-serif italic font-medium block sm:inline mt-2 sm:mt-0">
                  {cta.title}
                </span>
              </>
