...
export default function BusinessDetailsClient({ data }) {
   ...
   return (
      <section className="min-h-screen bg-white text-slate-900 selection:bg-blue-600 selection:text-white pb-40">
         ...
         <div className="border-b border-slate-100 bg-white/80 backdrop-blur-md sticky top-0 z-50">
            <div className="container-custom py-4 flex items-center justify-between">
               ...
               <div className="text-[10px] font-black uppercase tracking-[0.4em] text-blue-600">BUSINESS_SPEC_v4.0</div>
            </div>
         </div>
         ...
         <div className="container-custom pt-20">
            ...
            <div className="max-w-5xl mb-32 px-4 md:px-0">
               ...
               <p className="text-slate-500 text-xl font-light leading-relaxed max-w-2xl bg-slate-50 p-10 border-l-4 border-blue-600 rounded-r-3xl">
                  {hero.description}
               </p>
            </div>
               ...
               {roi?.map((spec, idx) => (
                  <div key={idx} className="p-10 border-b border-r border-slate-100 flex flex-col justify-between hover:bg-slate-50 transition-colors group">
                     <div className="flex justify-between items-start mb-16">
                        <div className="w-16 h-16 bg-blue-50 rounded-2xl flex items-center justify-center text-3xl text-blue-600 border border-blue-100 group-hover:bg-blue-600 group-hover:text-white transition-all">
                           {getIcon(spec.icon)}
                        </div>
                        <span className="text-[10px] font-black text-slate-300 group-hover:text-blue-200 transition-colors">KPI_SYNC_ACTIVE</span>
                     </div>
                     <div>
               ...
               <div className="relative group">
                  <div className="p-12 bg-slate-900 rounded-[4rem] text-white overflow-hidden relative shadow-2xl">
                     ...
                     <h3 className="text-3xl font-black mb-10 tracking-tighter flex items-center gap-4">
                        <IoHardwareChipOutline className="text-blue-500" /> Infrastructure Manifest
                     </h3>
                     <div className="space-y-4">
                        {manifest?.map((item, i) => (
                           <div key={i} className="flex justify-between items-center py-4 border-b border-white/5 group/row">
                              <span className="text-slate-500 text-[10px] font-black uppercase tracking-widest group-hover/row:text-blue-400 transition-colors">{item.label}</span>
            ...
            <div className="text-center py-40 border-t border-slate-100 px-4 md:px-0">
               <IoStatsChartOutline className="text-7xl text-blue-600 mb-12 mx-auto opacity-10" />
