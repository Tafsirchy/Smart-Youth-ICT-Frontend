...
export default function BusinessDetailsClient({ data }) {
   if (!data) return null;
   ...
   return (
      <section className="min-h-screen bg-white text-slate-900 selection:bg-blue-600 selection:text-white pb-12 md:pb-20 flex flex-col relative">
         ...
         <div className="border-b border-slate-100 bg-white/80 backdrop-blur-md sticky top-0 z-50">
            <div className="container-custom py-2 md:py-3 flex items-center justify-between">
               <Link href="/services/business-websites" className="flex items-center gap-2 text-[10px] font-black uppercase tracking-widest text-slate-400 hover:text-blue-600 transition-colors">
                  <IoArrowBackOutline className="text-sm" /> Infrastructure Overview
               </Link>
               <div className="text-[10px] font-black uppercase tracking-widest text-blue-600">BUSINESS_SPEC_v4.0</div>
            </div>
         </div>
         ...
         <div className="container-custom pt-6 md:pt-10 pb-[max(3rem,env(safe-area-inset-bottom))] md:pb-0">
            ...
            <div className="max-w-5xl mb-12 md:mb-16 px-4 md:px-0">
               <motion.div
                  initial={{ opacity: 0 }}
                  animate={{ opacity: 1 }}
                  className="flex items-center gap-3 md:gap-4 text-blue-600 mb-4"
               >
               ...
               <h1 className="text-3xl sm:text-5xl md:text-8xl font-black text-slate-900 leading-[1.1] md:leading-[0.9] mb-4 md:mb-6 tracking-tighter">
                  {hero.title} <br />
                  <span className="text-slate-200">{hero.subtitle}</span>
               </h1>
               <p className="text-slate-500 text-lg md:text-xl font-light leading-[1.6] max-w-2xl bg-slate-50 p-6 md:p-8 border-l-4 border-blue-600 rounded-r-2xl">
                  {hero.description}
               </p>
            </div>
               ...
               {roi?.map((spec, idx) => (
                  <div key={idx} className="p-6 md:p-8 border-b border-r border-slate-100 flex flex-col justify-between hover:bg-slate-50 transition-colors group">
                     <div className="flex justify-between items-start mb-8 md:mb-10">
                        <div className="w-12 h-12 md:w-16 md:h-16 bg-blue-50 rounded-xl md:rounded-2xl flex items-center justify-center text-2xl md:text-3xl text-blue-600 border border-blue-100 group-hover:bg-blue-600 group-hover:text-white transition-all">
                           {getIcon(spec.icon)}
                        </div>
            ...
            <div className="grid lg:grid-cols-2 gap-8 lg:gap-16 items-center mb-12 lg:mb-24 px-4 lg:px-0">
                     ...
                     <div className="space-y-2 md:space-y-3">
                        {manifest?.map((item, i) => (
                           <div key={i} className="flex justify-between items-center py-3 border-b border-white/5 group/row">
                              <span className="text-slate-500 text-[10px] font-black uppercase tracking-widest group-hover/row:text-blue-400 transition-colors leading-[1.4]">{item.label}</span>
                              <span className="text-sm font-bold text-white leading-[1.4]">{item.value}</span>
                           </div>
                        ))}
                     </div>
                  </div>
               ...
               <div>
                  ...
                  <div className="flex items-center gap-4 p-4 md:p-6 bg-blue-50 rounded-2xl md:rounded-3xl border border-blue-100">
                     <div className="w-10 h-10 md:w-12 md:h-12 bg-blue-600 rounded-lg md:rounded-xl flex items-center justify-center text-white text-lg md:text-xl shrink-0"><IoShieldCheckmarkOutline /></div>
                     <div>
                        ...
                        <p className="text-xs md:text-sm text-blue-600 font-bold leading-[1.4]">{sections.extraInfo?.securityValue || "Standard SSL + DDoS Shield Active"}</p>
                     </div>
                  </div>
               </div>
            </div>
            ...
            <div className="text-center py-12 lg:py-20 border-t border-slate-100 px-4 md:px-0">
               ...
               <div className="flex flex-col sm:flex-row gap-3 md:gap-4 justify-center">
                  <button className="w-full sm:w-[240px] min-h-[48px] px-4 py-3 md:py-4 bg-blue-600 text-white font-black rounded-xl hover:bg-blue-700 transition-all shadow-xl shadow-blue-600/40 uppercase tracking-widest text-[10px] flex items-center justify-center leading-[1.4]">
                     Initialize Build
                  </button>
                  <Link
                     href="/freelancing"
