...
export default function SocialCreativesDetailsClient({ data }) {
  if (!data) return null;

  const { hero, sections, cta } = data.details;

  return (
    <section className="min-h-screen bg-slate-50 text-slate-900 selection:bg-rose-600 selection:text-white pb-12 md:pb-20 relative font-sans flex flex-col">
      ...
      <div className="border-b border-slate-200 bg-white/80 backdrop-blur-md sticky top-0 z-50">
        <div className="container-custom py-2 md:py-3 flex items-center justify-between">
          <Link href="/services/social-creatives" className="flex items-center gap-2 text-[10px] font-black uppercase tracking-widest text-slate-400 hover:text-rose-600 transition-colors">
            <IoArrowBackOutline className="text-sm" /> Creative Overview
          </Link>
          <div className="text-[10px] font-black uppercase tracking-[0.4em] text-rose-600">CREATIVE_SPEC_v6.1</div>
        </div>
      </div>
      ...
      <div className="container-custom pt-6 md:pt-10 pb-[max(3rem,env(safe-area-inset-bottom))] md:pb-0">
        <div className="max-w-5xl mb-12 md:mb-16 relative">
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex items-center gap-3 md:gap-4 text-rose-600 mb-4 md:mb-6">
          ...
          <div className="flex flex-col md:flex-row gap-8 md:gap-12 items-start mt-8 md:mt-12">
            <div className="flex-1">
              ...
              <div className="h-1 w-12 md:w-20 bg-rose-600/20" />
            </div>
            <div className="w-full md:w-80 bg-white p-6 md:p-8 rounded-2xl md:rounded-3xl border border-slate-100 shadow-xl">
              <h4 className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-4 md:mb-6 px-1 leading-[1.4]">Engagement Pulse</h4>
        ...
        <div className="mb-12 lg:mb-24">
          <div className="flex items-center gap-4 md:gap-6 mb-8 md:mb-12">
            <h2 className="text-2xl md:text-3xl font-black text-slate-900 tracking-tighter uppercase leading-[1.1] italic">The Creative Protocol</h2>
            <div className="h-[1px] flex-1 bg-slate-200"></div>
            <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest leading-[1.4]">V6.1 Lifecycle</span>
          </div>
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-px bg-slate-200 border border-slate-200 rounded-2xl md:rounded-[3.5rem] overflow-hidden shadow-2xl">
            {(sections.phases || []).map((phase, idx) => (
              <div key={idx} className="bg-white p-6 md:p-10 hover:bg-slate-50 transition-all group relative overflow-hidden">
                <div className="absolute top-0 right-0 p-4 md:p-8 text-3xl md:text-4xl font-black text-slate-50 group-hover:text-rose-50 transition-colors select-none leading-none">{phase.step}</div>
                <div className="relative z-10">
                  <h4 className="text-[10px] font-black text-rose-600 uppercase tracking-[0.4em] mb-2 md:mb-4 leading-[1.4]">{phase.stage}</h4>
        ...
        <div className="mb-12 lg:mb-24">
          <div className="flex flex-col md:flex-row gap-6 md:gap-8 items-baseline mb-8 md:mb-12">
            <h2 className="text-[10px] font-black text-rose-600 uppercase tracking-[0.4em] border-l-4 border-rose-600 pl-3 md:pl-4 leading-[1.4]">Optimization Frameworks</h2>
          </div>
          ...
          <div className="grid lg:grid-cols-3 gap-4 md:gap-8">
            {(sections.roi || []).map((group, idx) => (
              <div key={idx} className="bg-white rounded-2xl md:rounded-[3rem] p-6 md:p-10 border border-slate-100 shadow-xl shadow-slate-200/50 hover:shadow-2xl transition-all group">
                <div className="w-10 h-10 md:w-12 md:h-12 bg-rose-50 rounded-xl flex items-center justify-center text-xl md:text-2xl text-rose-600 mb-6 md:mb-8 group-hover:scale-110 transition-transform"><IoFlashOutline /></div>
        ...
        <div className="text-center py-12 lg:py-20 border-t border-slate-200">
          <IoSparklesOutline className="text-5xl lg:text-7xl text-rose-600 mb-6 md:mb-8 mx-auto opacity-20" />
      ...
      <div className="sticky bottom-0 left-0 w-full p-[var(--gutter,16px)] bg-white/90 backdrop-blur-md border-t border-slate-100 z-50 md:hidden flex items-center justify-between shadow-[0_-10px_40px_rgba(0,0,0,0.05)] pb-[max(1rem,env(safe-area-inset-bottom))] mt-auto">
         <div className="flex flex-col gap-0.5">
            ...
            <p className="text-slate-900 font-bold text-sm leading-[1.4]">Start Creating</p>
         </div>
