...
export default function ErpCrmDetailsClient({ data }) {
   if (!data) return null;
   ...
   return (
      <section className="min-h-screen bg-white text-slate-900 selection:bg-indigo-600 selection:text-white pb-12 md:pb-20 flex flex-col relative">
         ...
         <div className="container-custom pt-6 md:pt-10 pb-[max(3rem,env(safe-area-inset-bottom))] md:pb-0">
            ...
            <div className="max-w-5xl mb-12 md:mb-16">
               ...
               <p className="text-slate-500 text-lg md:text-xl font-light leading-[1.6] max-w-2xl bg-slate-50 p-6 md:p-8 border-l-4 border-indigo-600 rounded-r-2xl">
                  {hero.description}
               </p>
            </div>
               ...
               {roi?.map((spec, idx) => (
                  <div key={idx} className="p-6 md:p-8 border-b border-r border-slate-100 flex flex-col justify-between hover:bg-slate-50 transition-colors group">
                     <div className="flex justify-between items-start mb-8 md:mb-10">
                        <div className="w-12 h-12 md:w-16 md:h-16 bg-indigo-50 rounded-xl md:rounded-2xl flex items-center justify-center text-2xl md:text-3xl text-indigo-600 border border-indigo-100 group-hover:bg-indigo-600 group-hover:text-white transition-all">
                           {getIcon(spec.icon)}
                        </div>
                        <span className="text-[10px] font-black text-slate-300 group-hover:text-indigo-200 transition-colors leading-[1.4]">KPI_SYNC_ACTIVE</span>
                     </div>
                     <div>
            ...
            <div className="grid lg:grid-cols-2 gap-8 lg:gap-16 items-center mb-12 lg:mb-24">
               <div className="relative group">
                  <div className="p-6 md:p-10 bg-slate-900 rounded-3xl md:rounded-[3rem] text-white overflow-hidden relative shadow-2xl">
                     ...
                     <h3 className="text-2xl md:text-3xl font-black mb-6 md:mb-8 tracking-tighter flex items-center gap-3 md:gap-4 leading-[1.1]">
                        <IoHardwareChipOutline className="text-indigo-500 shrink-0" /> Infrastructure Manifest
                     </h3>
                     <div className="space-y-2 md:space-y-3">
                        {manifest?.map((item, i) => (
                           <div key={i} className="flex justify-between items-center py-3 border-b border-white/5 group/row">
                              <span className="text-slate-500 text-[10px] font-black uppercase tracking-widest group-hover/row:text-indigo-400 transition-colors leading-[1.4]">{item.label}</span>
               ...
               <div>
                  <h2 className="text-3xl sm:text-4xl md:text-5xl font-black text-slate-900 mb-4 md:mb-6 tracking-tighter leading-[1.1]">Operational <br /> <span className="text-indigo-600 font-serif italic font-medium">Sovereignty.</span></h2>
                  <p className="text-slate-500 text-sm md:text-base font-light leading-[1.6] mb-6 md:mb-8">We deliver more than software. We deliver a high-velocity enterprise engine isolated from the common friction of monolithic third-party SaaS platforms.</p>
                  <div className="flex items-center gap-4 p-4 md:p-6 bg-indigo-50 rounded-2xl md:rounded-3xl border border-indigo-100">
                     <div className="w-10 h-10 md:w-12 md:h-12 bg-indigo-600 rounded-lg md:rounded-xl flex items-center justify-center text-white text-lg md:text-xl shrink-0"><IoShieldCheckmarkOutline /></div>
                     <div>
                        <p className="text-[10px] md:text-xs font-black text-indigo-900 uppercase tracking-widest leading-[1.4]">Security Protocol</p>
                        <p className="text-xs md:text-sm text-indigo-600 font-bold leading-[1.4]">End-to-End Encryption Active</p>
                     </div>
                  </div>
               </div>
            </div>
            ...
            <div className="mb-12 lg:mb-24">
               <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 md:mb-12 gap-4 md:gap-6">
                  <h2 className="text-3xl md:text-4xl font-black text-slate-900 tracking-tighter leading-[1.1]">Enterprise <span className="text-slate-400">Handover Artifacts.</span></h2>
            ...
            <div className="text-center py-12 lg:py-20 border-t border-slate-100">
               <IoStatsChartOutline className="text-5xl lg:text-6xl text-indigo-600 mb-6 md:mb-8 mx-auto opacity-10" />
