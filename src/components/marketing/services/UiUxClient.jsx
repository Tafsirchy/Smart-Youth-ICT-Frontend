    ...
    <section className="min-h-screen bg-slate-50 text-slate-900 selection:bg-cyan-600 selection:text-white overflow-hidden relative font-sans">
      ...
      <div className="container-custom py-20 relative z-10">
        ...
        <div className="flex flex-col lg:flex-row items-center gap-10 lg:gap-20 mb-20 lg:mb-48">
          <div className="flex-1 text-left">
            <motion.div
              ...
              className="inline-flex items-center gap-2 px-5 py-2 rounded-full bg-cyan-50 border border-cyan-100 text-cyan-700 text-[10px] sm:text-xs font-black tracking-[0.2em] sm:tracking-[0.4em] uppercase mb-10"
            >
            ...
            <div className="flex flex-col sm:flex-row gap-4 sm:gap-6">
              <button className="w-full sm:w-[280px] px-8 py-4 sm:py-6 bg-cyan-600 text-white font-black rounded-xl hover:bg-cyan-700 transition-all shadow-xl sm:shadow-2xl shadow-cyan-600/20 uppercase tracking-wider sm:tracking-widest text-xs sm:text-sm flex items-center justify-center min-h-[44px]">
                Initialize Product Audit
              </button>
              <Link href="/services/ui-ux/details" className="w-full sm:w-[280px] px-8 py-4 sm:py-6 bg-white border border-slate-200 text-slate-900 font-black rounded-xl hover:bg-slate-50 transition-all uppercase tracking-wider sm:tracking-widest text-xs sm:text-sm flex items-center justify-center text-center min-h-[44px]">
                Design Manifest
              </Link>
            </div>
          </div>
        ...
        <div className="mb-20 lg:mb-48">
          <div className="flex flex-col md:flex-row justify-between items-start md:items-end mb-12 lg:mb-24 gap-4 lg:gap-8 border-l-4 border-cyan-600 pl-6 lg:pl-8">
            <div className="max-w-xl">
              <h2 className="text-[10px] sm:text-xs font-black text-cyan-600 uppercase tracking-[0.2em] sm:tracking-[0.4em] mb-4 font-bold">Experience Philosophy</h2>
        ...
        <div className="mb-20 lg:mb-48">
          <div className="grid lg:grid-cols-2 gap-10 lg:gap-20 items-center bg-white rounded-3xl lg:rounded-[4rem] p-8 sm:p-12 lg:p-24 border border-slate-100 shadow-xl lg:shadow-2xl shadow-slate-200/50 relative overflow-hidden">
            ...
            <div className="relative z-10 space-y-8 sm:space-y-12">
              ...
              <h2 className="text-4xl sm:text-5xl lg:text-6xl font-black text-slate-900 leading-[1.1] sm:leading-[0.9] tracking-tighter sm:tracking-normal">Atomic <br className="hidden sm:block" /><span className="text-cyan-600 block sm:inline mt-2 sm:mt-0">Scaling.</span></h2>
              <p className="text-slate-500 text-lg sm:text-xl font-light leading-relaxed">We build highly scalable Design Systems that standardize component logic and ensure absolute visual consistency across platforms.</p>
              ...
              <div className="grid grid-cols-2 gap-px bg-slate-50 border border-slate-100 rounded-2xl lg:rounded-3xl overflow-hidden mt-8 lg:mt-10">
                {metrics?.map((item, idx) => (
                  <div key={idx} className="p-6 lg:p-10 hover:bg-white transition-colors group">
                    ...
                    <h4 className="text-[10px] sm:text-xs font-black text-cyan-600 uppercase tracking-wider lg:tracking-widest mb-2">{item.t}</h4>
                    <p className="text-xs sm:text-sm text-slate-400 font-bold italic">{item.d}</p>
                  </div>
                ))}
              </div>
            </div>
        ...
        <div className="text-center py-20 lg:py-40 border-t border-slate-200 pb-32 lg:pb-40">
          <IoColorPaletteOutline className="text-6xl lg:text-7xl text-cyan-600 mb-8 lg:mb-12 mx-auto opacity-20" />
          <h3 className="text-4xl sm:text-5xl lg:text-[5.5rem] font-black text-slate-900 mb-10 lg:mb-14 leading-[1.1] tracking-tighter">
            {cta.title?.includes('. ') ? (
              <>
                {cta.title.split('. ')[0]}. <br className="hidden sm:block" />
                <span className="text-transparent bg-clip-text bg-gradient-to-r from-cyan-600 to-indigo-600 font-serif italic font-medium block sm:inline mt-2 sm:mt-0">
                  {cta.title.split('. ')[1]}
                </span>
              </>
            ) : (
              <>
                Ready to activate your <br className="hidden sm:block" />
                <span className="text-transparent bg-clip-text bg-gradient-to-r from-cyan-600 to-indigo-600 font-serif italic font-medium block sm:inline mt-2 sm:mt-0">
                  {cta.title}
                </span>
              </>
          ...
          <div className="flex flex-col sm:flex-row gap-4 sm:gap-6 justify-center items-center">
            ...
            <div className="fixed bottom-0 left-0 w-full px-4 pt-4 pb-[max(1rem,env(safe-area-inset-bottom))] bg-white/90 backdrop-blur-md border-t border-slate-200 z-50 lg:relative lg:border-none lg:bg-transparent lg:p-0 lg:w-auto">
              <button className="w-full lg:w-[320px] px-8 py-4 sm:py-7 bg-cyan-600 text-white font-black rounded-xl lg:rounded-2xl hover:bg-cyan-700 transition-all shadow-xl sm:shadow-[0_20px_40px_-15px_rgba(6,182,212,0.3)] uppercase tracking-wider sm:tracking-[0.2em] text-xs sm:text-sm flex items-center justify-center min-h-[44px] gap-2 lg:gap-3 active:scale-95">
                <IoFlashOutline className="text-lg lg:text-xl" /> Initialize Product Audit
              </button>
            </div>
          </div>
        </div>
