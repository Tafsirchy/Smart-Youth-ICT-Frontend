...
export default function FacebookAdsDetailsClient({ data }) {
  ...
  return (
    <section className="min-h-screen bg-slate-50 text-slate-900 selection:bg-emerald-600 selection:text-white pb-40 relative font-sans">
      ...
      <div className="container-custom pt-20">
        <div className="max-w-5xl mb-32 px-4 md:px-0 relative">
          ...
          <div className="flex flex-col md:flex-row gap-12 items-start mt-20">
             <div className="flex-1">
                ...
                <div className="h-1 w-20 bg-emerald-600/20" />
             </div>
        ...
        <div className="mb-48 px-4 md:px-0">
          <div className="flex items-center gap-6 mb-16">
            <h2 className="text-3xl font-black text-slate-900 tracking-tighter uppercase leading-none italic">The Ads Protocol</h2>
            <div className="h-[1px] flex-1 bg-slate-200"></div>
            <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest leading-none">V6.4 Lifecycle</span>
          </div>
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-px bg-slate-200 border border-slate-200 rounded-[3.5rem] overflow-hidden shadow-2xl">
            {(sections.phases || []).map((phase, idx) => (
              <div key={idx} className="bg-white p-12 hover:bg-slate-50 transition-all group relative overflow-hidden">
                <div className="absolute top-0 right-0 p-8 text-4xl font-black text-slate-50 group-hover:text-emerald-50 transition-colors select-none">{phase.step}</div>
                <div className="relative z-10">
                   <h4 className="text-[10px] font-black text-emerald-600 uppercase tracking-[0.4em] mb-4">{phase.stage}</h4>
        ...
        <div className="mb-48 px-4 md:px-0">
           <div className="flex flex-col md:flex-row gap-12 items-baseline mb-20">
              <h2 className="text-[10px] font-black text-emerald-600 uppercase tracking-[0.4em] border-l-4 border-emerald-600 pl-4">Optimization Frameworks</h2>
           </div>
           ...
           <div className="grid lg:grid-cols-3 gap-8">
              {(sections.roi || []).map((group, idx) => (
                 <div key={idx} className="bg-white rounded-[3rem] p-12 border border-slate-100 shadow-xl shadow-slate-200/50 hover:shadow-2xl transition-all group">
                    <div className="w-12 h-12 bg-emerald-50 rounded-xl flex items-center justify-center text-2xl text-emerald-600 mb-8 group-hover:scale-110 transition-transform"><IoFlashOutline /></div>
