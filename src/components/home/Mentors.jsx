...
import { IoLogoLinkedin, IoLogoTwitter, IoSchoolOutline, IoBriefcaseOutline, IoExpandOutline } from 'react-icons/io5';
...
const MentorCard = ({ mentor }) => {
  ...
  return (
    ...
    >
      {/* Background Glow */}
      <div className={`absolute -inset-4 rounded-[48px] ${mentor.glowColor} blur-3xl opacity-0 group-hover:opacity-100 transition-opacity duration-700`} />
      ...
      <div className="relative h-full bg-slate-900/40 border border-white/10 backdrop-blur-3xl rounded-[40px] p-8 flex flex-col items-center text-center transition-all duration-500 group-hover:bg-slate-900/60 group-hover:border-white/20 overflow-hidden">
        ...
        <div className={`absolute top-0 right-0 w-24 h-24 bg-gradient-to-bl ${mentor.color} opacity-20 blur-2xl group-hover:opacity-40 transition-opacity`} />
        
        {/* Image Container with Floating Effect */}
        <div className="relative mb-10 w-40 h-40" style={{ transform: 'translateZ(50px)' }}>
          <div className={`absolute inset-0 bg-gradient-to-tr ${mentor.color} rounded-[48px] rotate-6 group-hover:rotate-12 transition-transform duration-500 blur-md opacity-50`} />
          <div className={`absolute inset-0 border-2 border-white/20 rounded-[48px] -rotate-3 group-hover:-rotate-6 transition-transform duration-500`} />
        ...
        <div className="relative z-10 space-y-4" style={{ transform: 'translateZ(30px)' }}>
          <div>
            <h3 className="text-3xl font-black text-white tracking-tight mb-1">{mentor.name}</h3>
        ...
        {/* Socials & Action */}
        <div className="mt-8 flex items-center justify-between w-full relative z-10" style={{ transform: 'translateZ(20px)' }}>
          <div className="flex gap-3">
            <a href="#" className="w-10 h-10 rounded-2xl bg-white/5 border border-white/10 flex items-center justify-center text-white/40 hover:bg-white hover:text-slate-900 transition-all duration-300">
          ...
          <button className="flex items-center gap-2 text-[10px] font-black text-white/40 uppercase tracking-widest hover:text-white transition-colors group/btn">
            View Profile <IoExpandOutline className="group-hover/btn:rotate-45 transition-transform" />
          </button>
        </div>
      </div>
    </motion.div>
      ...
      <div className="container-custom relative z-10">
        ...
        >
          ...
          <h2 className="text-5xl md:text-7xl font-black text-white leading-[1.1] mb-8 tracking-tighter">
            Learn from the <br/>
            <span className="text-transparent bg-clip-text bg-gradient-to-r from-pink-500 via-rose-500 to-amber-500 animate-gradient-x">Best in Industry</span>
          </h2>
          
          <p className="text-white/50 text-xl font-medium max-w-2xl mx-auto leading-relaxed">
            Direct guidance from top-tier professionals who have built, scaled, and transformed 
