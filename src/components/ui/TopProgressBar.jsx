...
export default function TopProgressBar({ debugMode = false }) {
  ...
  const completeBar = useCallback((elapsed) => {
    clearTimeout(hideTimerRef.current);

    // Pick color by load time
    const color =
      elapsed >= CRITICAL_MS ? COLOR_CRIT
      : elapsed >= WARN_MS   ? COLOR_WARN
      : COLOR_GOOD;
    ...
    if (debugMode && elapsed != null) {
      const level =
        elapsed >= CRITICAL_MS ? 'critical'
        : elapsed >= WARN_MS   ? 'warn'
        : 'good';
  ...
  useEffect(() => {
    const handleClick = (e) => {
      ...
      if (isInternal && !isHashOnly && !isSamePage && !isDownload && !isNewTab) {
        startBar();
      }
    };
  ...
  const transition = barState === 'completing'
...
function DebugBadge({ debugInfo, onClose }) {
  ...
  const label =
    level === 'critical' ? `Slow! Consider code splitting & caching.`
    : level === 'warn'   ? `Slightly slow. Check API & image sizes.`
    : `Fast navigation ✓`;
  ...
  return (
    ...
    >
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '4px' }}>
        ...
        <span style={{ color: c.border, fontSize: '10px', textTransform: 'uppercase', letterSpacing: '0.08em' }}>
          route load
        </span>
      </div>

      {/* Label */}
      <div style={{ color: c.text, fontSize: '11px', opacity: 0.85 }}>{label}</div>
