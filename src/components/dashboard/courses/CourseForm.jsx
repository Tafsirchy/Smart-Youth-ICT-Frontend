...
export default function CourseForm({ initialData = null, onSuccess }) {
  ...
  const [formData, setFormData] = useState({
    ...
    certification: {
      included: true,
      desc: "Earn an industry-recognized certificate",
    },
  ...
  useEffect(() => {
    if (initialData) {
      setFormData({
        ...
        targetAudience: initialData.targetAudience?.length
          ? initialData.targetAudience
          : [""],
        features: initialData.features?.length
          ? initialData.features
          : [{ iconKey: "FaCheck", text: "" }],
        faqs: initialData.faqs?.length
          ? initialData.faqs
          : [{ question: "", answer: "" }],
        projects: initialData.projects?.length
          ? initialData.projects
          : [{ title: "", desc: "", techs: [], image: "" }],
        curriculum: initialData.curriculum?.length
          ? initialData.curriculum
          : [{ title: "", duration: "", isFree: false, topics: [""] }],
      });
    }
  ...
  const removeTopic = (modIdx, topicIdx) => {
    const newCurr = [...formData.curriculum];
    newCurr[modIdx].topics = newCurr[modIdx].topics.filter(
      (_, i) => i !== topicIdx,
    );
    setFormData({ ...formData, curriculum: newCurr });
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      data.append(
        "outcomes",
        JSON.stringify(formData.outcomes.filter((x) => x.trim())),
      );
      data.append(
        "targetAudience",
        JSON.stringify(formData.targetAudience.filter((x) => x.trim())),
      );
      data.append(
        "features",
        JSON.stringify(formData.features.filter((x) => x.text.trim())),
      );
      data.append(
        "faqs",
      ...
      data.append(
        "curriculum",
        JSON.stringify(formData.curriculum.filter((x) => x.title.trim())),
      );
    ...
    <div className="flex justify-between items-center mb-4">
      ...
      {onAdd && (
        <button
          type="button"
          onClick={onAdd}
          className="btn-sm bg-neutral-100 flex items-center gap-1 px-3 mt-1 rounded-full text-sm font-semibold hover:bg-neutral-200"
        >
          <FiPlus /> Add
        </button>
      )}
  ...
  return (
    <form
      ...
      className="bg-white rounded-3xl shadow-sm border border-neutral-200 p-6 md:p-10"
    >
      {/* Tabs */}
      ...
      {error && (
        <div className="mb-6 bg-red-50 text-red-600 p-4 rounded-xl font-semibold border border-red-100 flex items-center gap-2">
          <FiX /> {error}
        </div>
      )}
      ...
      <div className="min-h-[500px]">
          ...
          >
            <div className="grid md:grid-cols-2 gap-6">
              <div>
                ...
                <input
                  required
                  type="text"
                  className="w-full border rounded-xl p-3 focus:ring-2 outline-none focus:ring-blue-500"
            ...
            <div className="grid md:grid-cols-3 gap-6">
              <div>
                ...
                <select
                  className="w-full border rounded-xl p-3 focus:ring-2 outline-none focus:ring-blue-500 bg-white"
            ...
            <div className="flex items-center gap-3 bg-neutral-50 p-4 rounded-xl border border-neutral-100">
              <input
                type="checkbox"
                id="popular"
                className="w-5 h-5 rounded cursor-pointer accent-pink-600"
            ...
            <div className="border-2 border-dashed border-neutral-300 rounded-3xl p-10 text-center relative hover:bg-neutral-50 hover:border-blue-400 transition-colors bg-white">
              ...
              {thumbnailFile ? (
                <div className="text-emerald-600 font-semibold flex items-center justify-center gap-2">
                  <FiCheckCircle size={24} /> File selected:{" "}
                  {thumbnailFile.name}
                </div>
          ...
          >
            ...
            <div className="space-y-6">
              {formData.curriculum.map((mod, i) => (
                <div
                  key={i}
                  className="border rounded-2xl p-5 bg-neutral-50/50 shadow-sm relative group overflow-hidden"
                >
                  <div className="flex flex-wrap gap-4 mb-4 items-end">
                    <div className="flex-1 min-w-[200px]">
                      ...
                      <input
                        ...
                        className="w-full border rounded-xl p-3 mt-1.5 focus:ring-2 outline-none focus:ring-blue-500 bg-white"
                        value={mod.title}
                        onChange={(e) =>
                          handleObjectArray(
                            "curriculum",
                    ...
                    <button
                      type="button"
                      onClick={() => removeArrayItem("curriculum", i)}
                      className="p-3.5 bg-white border border-red-200 text-red-500 rounded-xl hover:bg-red-50 transition-colors"
                    >
                      <FiTrash2 size={18} />
                    </button>
                  </div>
                  ...
                  <div className="pl-6 border-l-2 border-blue-200 space-y-3 mt-6">
                    <p className="text-xs font-bold text-blue-600 uppercase tracking-wider mb-2">
                      Lessons / Topics inside Module {i + 1}
                    </p>
                    ...
                    <button
                      type="button"
                      onClick={() => addTopic(i)}
                      className="text-sm text-blue-600 font-bold mt-4 flex items-center gap-1 hover:text-blue-800"
                    >
        ...
        {activeTab === "Features & Outcomes" && (
          ...
          >
                ...
                {formData.outcomes.map((out, i) => (
                  <div key={i} className="flex gap-2 items-center">
                    ...
                    <button
                      ...
                      className="p-2 text-red-400 hover:text-red-600 rounded-xl"
                    >
                      <FiTrash2 />
                    </button>
                  </div>
            ...
            <div>
              <SectionHeader
                title="Key Features (Grid Icons)"
                onAdd={() =>
                  addArrayItem("features", { iconKey: "FaCheck", text: "" })
                }
              />
              <div className="space-y-4">
                {formData.features.map((feat, i) => (
                  ...
                  >
                    <div className="shrink-0 w-12 h-12 bg-blue-50 text-blue-600 rounded-lg flex items-center justify-center font-bold text-xs shadow-inner">
                      ICON
                    </div>
                    <input
                      type="text"
          ...
          >
            ...
            <div className="grid md:grid-cols-2 gap-6">
              {formData.projects.map((proj, i) => (
                <div
                  key={i}
                  className="border rounded-2xl p-6 bg-white shadow-sm ring-1 ring-neutral-200 relative group overflow-hidden"
                >
                  <button
                    type="button"
                    onClick={() => removeArrayItem("projects", i)}
                    className="absolute top-4 right-4 p-2 bg-red-50 text-red-500 rounded-lg opacity-0 group-hover:opacity-100 transition-opacity"
                  >
                    <FiTrash2 />
                  </button>

                  <label className="block text-xs font-bold text-neutral-500 uppercase tracking-widest mb-1 mt-2">
                    Project Title
                  </label>
                  <input
                    type="text"
                  ...
                  <textarea
                    ...
                    value={proj.desc}
                    onChange={(e) =>
                      handleObjectArray("projects", i, "desc", e.target.value)
                    }
                  />
                  ...
                  <input
                    ...
                    className="w-full border rounded-xl p-3 font-mono text-sm focus:ring-2 outline-none focus:ring-blue-500 bg-neutral-50"
          ...
          >
            ...
            <div className="space-y-4">
              {formData.faqs.map((faq, i) => (
                <div
                  key={i}
                  className="flex gap-4 items-start border rounded-2xl p-5 bg-white shadow-sm ring-1 ring-neutral-200 transition-all hover:ring-blue-300"
                >
                  <div className="flex-1 space-y-3">
                    ...
                    <textarea
                      ...
                      className="w-full border rounded-xl p-3 text-sm text-neutral-600 focus:ring-2 outline-none focus:ring-blue-500"
                      value={faq.answer}
                      onChange={(e) =>
                        handleObjectArray("faqs", i, "answer", e.target.value)
                      }
                    />
                  </div>
                  <button
                    ...
                    className="p-3 text-red-500 hover:bg-red-50 rounded-xl mt-1 shrink-0"
                  >
                    <FiTrash2 size={20} />
                  </button>
                </div>
      ...
      <div className="mt-8 pt-6 border-t border-neutral-200 flex flex-wrap justify-between items-center gap-4">
        {initialData && (
          <span className="text-sm font-semibold text-neutral-500 border px-3 py-1 rounded-full">
        ...
        <div className="flex gap-3 ml-auto">
          ...
          <button
            type="submit"
            disabled={loading}
            className="px-8 py-3 rounded-full font-bold text-white bg-blue-600 hover:bg-blue-700 shadow-md shadow-blue-500/20 transition-all disabled:opacity-50 flex items-center gap-2"
          >
            <FiSave size={18} />
            {loading
              ? "Saving to Database..."
